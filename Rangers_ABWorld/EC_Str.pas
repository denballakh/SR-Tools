...
begin
    ...
    for i:=0 to cnt-2 do begin
    	for u:=i+1 to cnt-1 do begin
        	if ComparerStrEC(PWideChar(Item[i]),PWideChar(Item[u]))>0 then begin
            	tstr:=Item[i]; Item[i]:=Item[u]; Item[u]:=tstr;
            end;
        end;
    end;
end;
...
begin
    ...
    for i:=1 to len do begin
        ...
        end else if (integer(str[i])>=integer('a')) and (integer(str[i])<=integer('f')) then begin
...
begin
    Result:=tstr;
    tlen:=Length(Result);
    if tlen>0 then CharUpperBuffW(PWideChar(Result),tlen);
end;
...
begin
    Result:=tstr;
    tlen:=Length(Result);
    if tlen>0 then CharLowerBuffW(PWideChar(Result),tlen);
end;
