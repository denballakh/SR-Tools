...
begin
  ...
  begin
    atr2 := atr;
    atr := atr.FNext;
    if atr2.FOwner = self then
      Triangle_Delete(atr2);
  end;
  ...
  if FGraphSel <> nil then
  begin
    ab_Obj3D_Delete(FGraphSel);
    FGraphSel := nil;
  end;
  if FGraphSelBB <> nil then
  begin
...
begin
  ...
  begin
    ...
    if (FGraphSelBB = nil) and (FBBState = 1) then
    begin
...
begin
  ...
  while apo <> nil do
  begin
    if apo.FOwner.IndexOf(self) >= 0 then
      Inc(result);
    apo := apo.FNext;
  end;
end;
...
begin
  ...
  while atr <> nil do
  begin
    if atr.FOwner = self then
      if (atr.FV[0].FVer = apo) or (atr.FV[1].FVer = apo) or (atr.FV[2].FVer = apo) then
      begin
        result := true;
        exit;
      end;
...
begin
  ...
  if FKeyGroup < 0 then
    FKeyGroup := 0
  else if FKeyGroup >= KeyGroupList_Get(self).FList.Count then
    FKeyGroup := KeyGroupList_Get(self).FList.Count - 1;
...
begin
  ...
  for i := 0 to cnt - 1 do
  begin
    ab_CalcAngleAndDist(center_orb, center_orbangle, 0, p_list[i].p.FOrbit, p_list[i].p.FOrbitAngle,
      ab_WorldRadius, t_angle, t_radius);
    if t_angle < 0 then
      t_angle := 360 + t_angle;
  ...
  FBB[0].FOrbit := center_orb;
  FBB[0].FOrbitAngle := center_orbangle;
  FBB[1].FOrbit := center_orb;
  FBB[1].FOrbitAngle := center_orbangle;
  FBB[2].FOrbit := center_orb;
  FBB[2].FOrbitAngle := center_orbangle;
  FBB[3].FOrbit := center_orb;
  FBB[3].FOrbitAngle := center_orbangle;
...
begin
  ...
  begin
    apo := lp.Items[i];
    v.x := apo.FPos.x - center.x;
    v.y := apo.FPos.y - center.y;
    v.z := apo.FPos.z - center.z;
...
procedure WorldUnit_Clear;
begin
  while WorldUnit_First <> nil do
    WorldUnit_Delete(WorldUnit_Last);
end;
...
begin
  ...
  if WorldUnit_Last <> nil then
    WorldUnit_Last.FNext := el;
  el.FPrev := WorldUnit_Last;
  el.FNext := nil;
  WorldUnit_Last := el;
  if WorldUnit_First = nil then
    WorldUnit_First := el;
...
begin
  ...
  if WorldUnit_Last = el then
    WorldUnit_Last := el.FPrev;
  if WorldUnit_First = el then
    WorldUnit_First := el.FNext;
...
begin
  ...
  begin
    ...
    if (wp.FUnit = own) and (wp.FId = id) then
    begin
      result := wp;
      exit;
    end;
...
begin
  ...
  begin
    ...
    while u < wtypecnt do
    begin
      if wp.FId = TrimEx(GetStrParEC(wtype, u, ',')) then
        break;
      Inc(u);
    end;
...
begin
  ...
  while apo <> nil do
  begin
    if (apo.FOwner.Count <> 1) or (apo.FPortId = '') or (apo.FPortType = '') or
      (apo.FPortLink = '') then
    begin
      apo := apo.FNext;
      continue;
    ...
    begin
      if Length(wp.FType) < Length(apo.FPortType) then
        wp.FType := apo.FPortType;
      Inc(wp.FPointCnt);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if D3D_DotProduct(l1, l2) >= 0 then
        begin
          md := td;
          wpcur := wpcur2;
          wpdes := wpdes2;
    ...
    if (wpcur = nil) or (wpdes = nil) then
      break;
    ...
    begin
      ...
      if apo1.FOwner.Count <> 1 then
        continue;
      if apo1.FOwner.IndexOf(wunit) < 0 then
        continue;
      if apo1.FPortId <> wpcur.FId then
        continue;

      apo2 := Point_First;
      while apo2 <> nil do
      begin
        if (apo2.FOwner.Count = 1) and (apo2.FOwner.IndexOf(wpdes.FUnit) >= 0) and
          (apo2.FPortId = wpdes.FId) and (apo1.FPortLink = apo2.FPortLink) then
          break;
        apo2 := apo2.FNext;
      end;
...
begin
  ...
  while apo <> nil do
  begin
    if apo.FOwner.IndexOf(own) >= 0 then
      if FindSubstring(apo.FPortId, id) >= 0 then
      begin
        result := apo;
        exit;
      end;
...
begin
  ...
  begin
    ...
    (apo.FPortId = '') or (apo.FPortLink = '') then
    begin
      apo := apo.FNext;
      continue;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (td > 0.001) and (td < sqr(100)) and (wpdes2 <> nil) and (td < md) then
      begin
        l1 := Dxyz(wpcur2.FCenter.x - center.x, wpcur2.FCenter.y - center.y, wpcur2.FCenter.z - center.z);
        l2 := Dxyz(wpdes2.FCenter.x - center.x, wpdes2.FCenter.y - center.y, wpdes2.FCenter.z - center.z);
        if D3D_DotProduct(l1, l2) >= 0 then
