...
begin
    ...
    if Key_Last<>nil then Key_Last.FKeyNext:=self;
	self.FKeyPrev:=Key_Last;
	self.FKeyNext:=nil;
	Key_Last:=self;
	if Key_First=nil then Key_First:=self;
...
var
	i,u:integer;
begin
	if self.FKeyPrev<>nil then self.FKeyPrev.FKeyNext:=self.FKeyNext;
	if self.FKeyNext<>nil then self.FKeyNext.FKeyPrev:=self.FKeyPrev;
	if Key_Last=self then Key_Last:=self.FKeyPrev;
	if Key_First=self then Key_First:=self.FKeyNext;

    for i:=0 to High(FUnit) do begin
	    for u:=0 to FCount-1 do begin
        	FreeEC(FUnit[i][u]);
        end;
        FUnit[i]:=nil;
    ...
    if FInterpolateVal<>nil then begin
...
begin
    ...
    for i:=0 to FGroupList.Count-1 do begin
    	SetLength(FUnit[i],FCount);
        for u:=FCount-1 downto no+1 do FUnit[i][u]:=FUnit[i][u-1];

        FUnit[i][no]:=AllocEC(FUnitSize);
        if no<(FCount-1) then begin
    ...
    SetLength(FUnitT,FCount);
    for u:=FCount-1 downto no+1 do FUnitT[u]:=FUnitT[u-1];
	if no<(FCount-1) then FUnitT[no]:=FUnitT[no+1]
    else FUnitT[no]:=FUnitT[no-1];
end;
...
var
	i,u:integer;
begin
	if (no<0) or (no>=FCount) or (FCount<=1) then EError('TKeyAB.KeyDelete');

    for i:=0 to FGroupList.Count-1 do begin
	    FreeEC(FUnit[i][no]);
    	for u:=no+1 to FCount-1 do FUnit[i][u-1]:=FUnit[i][u];
        SetLength(FUnit[i],FCount-1);
    end;

	for u:=no+1 to FCount-1 do FUnitT[u-1]:=FUnitT[u];
	SetLength(FUnitT,FCount-1);

    dec(FCount);

    if FCur>=FCount then FCur:=FCount-1;
end;
...
begin
    ...
    for i:=High(FUnit) downto no+1 do begin
    	{PDWORD(Pointer(}FUnit[i]{))^}:={PDWORD(Pointer(}FUnit[i-1]{))^};
    end;
//    PDWORD(Pointer(FUnit[no]))^:=0;
    SetLength(FUnit[no],FCount);
    for i:=0 to FCount-1 do begin
    	FUnit[no][i]:=AllocEC(FUnitSize);
        if no<(High(FUnit)+1-1) then begin
...
begin
    ...
    FUnit[no]:=nil;
    for i:=no+1 to High(FUnit) do begin
    	{PDWORD(Pointer(}FUnit[i-1]{))^}:={PDWORD(Pointer(}FUnit[i]{))^};
    end;
...
begin
    ...
    if keytimelength>=wu.FTimeLength then begin
    	ztime:=Round(ztime) mod keytimelength;
    end else begin
    	ztime:=Round(ztime) mod wu.FTimeLength;

        if ztime<(wu.FTimeLength-keytimelength) then begin
        	i1:=0; i2:=0; t:=0;
            Exit;
        end else begin
	...
	i1:=0;
	while True do begin
        if (ztime-FUnitT[i1])<0 then break;
        if FUnitT[i1]<=0 then inc(zerocnt);
    	ztime:=ztime-FUnitT[i1];
        inc(i1);
        if i1>=FCount then begin
        	if zerocnt=FCount then begin i1:=0; i2:=0; t:=0; Exit; end;
        	zerocnt:=0;
        	i1:=0;
        end;
    end;

    i2:=i1+1; if i2>=FCount then i2:=0;
    t:=ztime/FUnitT[i1];
end;
...
begin
	Result:=0;
	for i:=0 to cnt-1 do Result:=Result+FUnitT[i];
end;
...
begin
    ...
    for i:=0 to FGroupList.Count-1 do begin
    	for u:=0 to FCount-1 do begin
            ...
            buf:=Ptr(DWORD(buf)+1);
        	for t:=1 to FUnitSize-1 do begin
            	Result:=Result+' '+ByteToStrHexEC(PBYTE(buf)^);
	            buf:=Ptr(DWORD(buf)+1);
            end;
        end;
...
begin
	sme:=0;
	cnt:=StrToIntEC(GetStrParEC(tstr,sme,',')); inc(sme);

    while FCount<cnt do KeyInsert(FCount);
    while FCount>cnt do KeyDelete(FCount-1);

    FTimeFull:=0;
    for i:=0 to FCount-1 do begin
        FUnitT[i]:=StrToIntEC(GetStrParEC(tstr,sme,',')); inc(sme);
        FTimeFull:=FTimeFull+FUnitT[i];
    end;
    for i:=0 to FGroupList.Count-1 do begin
    	for u:=0 to FCount-1 do begin
		    zs:=TrimEx(GetStrParEC(tstr,sme,',')); inc(sme);

        	cnt:=GetCountParEC(zs,' ');
            if cnt<>FUnitSize then EError('TKeyAB.Load');
            buf:=FUnit[i][u];
            for t:=0 to FUnitSize-1 do begin
...
var
	k:TKeyAB;
begin
	if (no<0) or (no>Count) then begin Result:=nil; Exit; end;
	Result:=TKeyGroup.Create;
	FList.Insert(no,Result);

    k:=Key_First;
    while k<>nil do begin
    	if k.FGroupList=self then begin
	    	k.GroupInsert(no);
        end;
    	k:=k.FKeyNext;
    end;
...
var
	k:TKeyAB;
begin
	if (no<0) or (no>=Count) or (Count<=1) then Exit;
	FList.Delete(no);
...
begin
    ...
    for i:=0 to cnt-1 do begin
        ...
        if tbp2.Par_Count('Name')>0 then begin
...
begin
	i:=0;
	while i<KeyGroupList_List.Count do begin
    	if TKeyGroupList(KeyGroupList_List.Items[i]).FOwner=ow then begin
...
var
	i:integer;
begin
	for i:=0 to KeyGroupList_List.Count-1 do begin
		if TKeyGroupList(KeyGroupList_List.Items[i]).FOwner=ow then begin
...
begin
	for i:=0 to KeyGroupList_List.Count-1 do begin
		if TKeyGroupList(KeyGroupList_List.Items[i]).FNo=no then begin
        	Result:=TKeyGroupList(KeyGroupList_List.Items[i]);
            Exit;
        end;
    end;
...
begin
    k:=Key_First;
    while k<>nil do begin
    	if TabWorldUnit(k.FGroupList.FOwner).FDraw then begin
			k.Interpolate;
        end;
    	k:=k.FKeyNext;
    end;
...
begin
    ...
    key.IpDWORD:=
    	iv(zn1 and $ff,zn2 and $ff) or
        (iv((zn1 shr 8) and $ff,(zn2 shr 8) and $ff) shl 8) or
