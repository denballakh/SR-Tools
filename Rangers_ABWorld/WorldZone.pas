...
var
  zl, zl2: TabZoneLink;
begin
  if FGraphLine <> nil then
  begin
    ab_Obj3D_Delete(FGraphLine);
    FGraphLine := nil;
  end;
  if FGraphCenter <> nil then
  begin
  ...
  begin
    zl2 := zl;
    zl := zl.FNext;
    if (zl2.FStart = self) or (zl2.FEnd = self) then
      ZoneLink_Delete(zl2);
  end;
end;
...
begin
  ...
  if FType = 0 then
    ...
    col := D3DCOLOR_ARGB(255, 0, 255, 255)//D3DCOLOR_ARGB(255,0,97,201)
  else if FType in [2, 3, 4] then
    col := D3DCOLOR_ARGB(255, 191, 94, 140{180,0,0})
  else if FType = 5 then
    col := D3DCOLOR_ARGB(255, 0, 255, 0)
  else if FType in [6, 7, 8] then
    col := D3DCOLOR_ARGB(255, 255, 0, 0)
  else if FType = 9 then
    col := D3DCOLOR_ARGB(255, 255, 255, 0)
  else
    col := D3DCOLOR_ARGB(255, 255, 150, 150);
  ...
  begin
    ...
    with ab_CalcNewPos(abPos(FOrb, FOrbAngle, angle), FRadius) do
...
begin
  ...
  FGraph.FDrawFromObjLoop := ab_InTop(FStart.FPosShow.z) and ab_InTop(FEnd.FPosShow.z) and
...
procedure Zone_Clear;
begin
  while Zone_First <> nil do
    Zone_Delete(Zone_Last);
end;
...
begin
  ...
  if Zone_Last = el then
    Zone_Last := el.FPrev;
  if Zone_First = el then
    Zone_First := el.FNext;
...
procedure ZoneLink_Clear;
begin
  while ZoneLink_First <> nil do
    ZoneLink_Delete(ZoneLink_Last);
end;
...
begin
  ...
  if ZoneLink_Last <> nil then
    ZoneLink_Last.FNext := el;
  el.FPrev := ZoneLink_Last;
  el.FNext := nil;
  ZoneLink_Last := el;
  if ZoneLink_First = nil then
    ZoneLink_First := el;
...
begin
  ...
  if el.FNext <> nil then
    el.FNext.FPrev := el.FPrev;
  if ZoneLink_Last = el then
    ZoneLink_Last := el.FPrev;
  if ZoneLink_First = el then
    ZoneLink_First := el.FNext;
...
begin
  ...
  while el <> nil do
  begin
    if (el.FStart = z1) and (el.FEnd = z2) then
    begin
      result := el;
      exit;
    end;
