...
procedure ab_CalcAngleAndDist(sou_orb,sou_orban,sou_angle,des_orb,des_orban,radius:xyzV; var r_ang:xyzV; var r_dist:xyzV); overload;
...
begin
    ...
    if b_<0.00001 then t:=99999999
    else t:=1/sin(b_);

    t2:=(sin(c_)*cos(a_)-cos(c_)*sin(a_)*cos(B))*t;
    if t2<-1.0 then t2:=-1.0 else if t2>1.0 then t2:=1.0;
    A:=arccos(t2);
...
procedure ab_CalcAngleAndDist(sou_orb,sou_orban,sou_angle,des_orb,des_orban,radius:xyzV; var r_ang:xyzV; var r_dist:xyzV);
var
    a_,b_,c_,A,B,t:xyzV;
begin
    ...
    if t<-1.0 then t:=-1.0 else if t>1.0 then t:=1.0;
    a_:=arccos(t);
    r_dist:=(a_/(2*pi))*2*pi*radius;

    if a_=0 then begin r_ang:=0; Exit; end;
    t:=(sin(c_)*cos(b_)-cos(c_)*sin(b_)*cos(A))/sin(a_);
    if t<-1.0 then t:=-1.0 else if t>1.0 then t:=1.0;
    B:=arccos(t);

    if AngleDist(sou_orb,des_orb)<0 then begin
        B:=-B;
    end;
...
begin
    ab_CalcAngleAndDist(sou.FOrbit,sou.FOrbitAngle,sou.FAngle,des.FOrbit,des.FOrbitAngle,ab_WorldRadius,Result.FAngle,Result.FDist);
end;
...
//    m2:=D3D_ProjectionMatrix(1,ab_WorldRadius*2.1,Angle360ToRad(ab_Camera_Fov),FormMain.Panel3D.Width);
    m2:=D3D_ProjectionMatrix(ab_Camera_Radius-ab_WorldRadius-100,ab_Camera_Radius+ab_WorldRadius+100,Angle360ToRad(ab_Camera_Fov),FormMain.Panel3D.Width);
...
begin
    ...
    if t2hc<=0.0 then begin Result:=false; Exit; End;
    t2hc:=sqrt(t2hc);
    if tca<t2hc then begin
        t:=tca+t2hc;
        t2:=tca-t2hc;
    end else begin
        t:=tca-t2hc;
...
{    if RegOpenKeyExW(HKEY_CURRENT_USER,PWChar(RegUserPath+path),0,KEY_READ,kkey)<>ERROR_SUCCESS then begin
    ...
    tsize:=4;
    if RegQueryValueExA(kkey,PChar(AnsiString(name)),nil,@tip,Windows.PByte(@zn),@tsize)<>ERROR_SUCCESS then begin
        Result:=bydef;
        RegCloseKey(kkey);
        Exit;
    end;
