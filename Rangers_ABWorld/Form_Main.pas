...
type
  TFormMain = class(TForm)
    ...
    procedure EditMainWorldRadiusKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
begin
	el:=SE_PointBuf.Buf;
	for i:=0 to SE_PointCnt-1 do begin
        if (el.FOrbit=orbit) and (el.FOrbitAngle=orbitangle) and (el.FRadius=radius) and (el.FU=tu) and (el.FV=tv) and (el.FColor=color) then begin
        	Result:=i;
            Exit;
        end;
...
begin
    ...
    if keytimelength<=wu.FTimeLength then keytimelength:=wu.FTimeLength;
...
var
    ...
    ztype:integer;
begin
    if TabWorldUnit(key.FGroupList.FOwner).FType=0 then begin
		des.Len:=16+1*8;
    	des.S(0,integer(0));
    	...
    	for i:=0 to key.FGroupList.FList.Count-1 do begin
	    	key.FGroupList.FCur:=i;

            ztype:=0;
            stype:=TrimEx(TKeyGroup(key.FGroupList.FList.Items[i]).FName);
            if IsIntEC(stype) then ztype:=StrToIntEC(stype);
...
begin
    ...
    sme:=0;
    while sme<SE_ColorKeyBuf.Len do begin
    	len:=SE_ColorKeyBuf.GetDWORD(sme+12);
        if len=lent then begin
        	if CompareMem(Ptr(DWORD(SE_ColorKeyBuf.Buf)+DWORD(sme)),tempbuf.Buf,len) then begin
            	Result:=sme;
                Exit;
            end;
        end;
...
//	timeBeginPeriod(1);
    ...
    if FileExists(GRangersPath+'\CFG\CD\ABWall.txt') then begin
...
begin
    ...
    KeyGroupList_List.Free; KeyGroupList_List:=nil;
...
begin
    ...
    ed:=(ActiveControl<>nil) and ((ActiveControl.ClassName='TEdit') or (ActiveControl.ClassName='TComboBox')) or (not Active);

	Handled:=false;
    if (Msg.message=($400+123)) and (not FStopDraw) then begin
...
//        Handled:=true;
    	...
    	if (Msg.wParam=VK_DELETE) and (WorldUnit_Sel<>nil) then begin
        	WorldUnit_Delete(WorldUnit_Sel);

    	end else if (Msg.wParam=VK_DELETE) and (Zone_Sel<>nil) then begin
        	Zone_Delete(Zone_Sel);

    	end else if (Msg.wParam=VK_DELETE) and (ZoneLink_Sel<>nil) then begin
        	ZoneLink_Delete(ZoneLink_Sel);

    	end else if (Msg.wParam=VK_SPACE) and (WorldUnit_Sel<>nil) then begin
            WorldPort2_Connect(WorldUnit_Sel);

    	end else if (_ctrl) and (Msg.wParam=integer('P')) then begin
	        MM_View_PointClick(MM_View_Point);

    	end else if (_ctrl) and (Msg.wParam=integer('W')) then begin
            MM_View_WorldLineClick(MM_View_WorldLine);

    	end else if (_ctrl) and (Msg.wParam=integer('B')) then begin
            MM_View_BackgroundClick(MM_View_Background);

    	end else if (_ctrl) and (Msg.wParam=integer('Z')) then begin
            MM_View_ZoneClick(MM_View_Zone);

    	end else if (Msg.wParam=integer('C')) and (WorldUnit_Sel<>nil) then begin
        	wunit:=WorldUnit_Sel;
        	GetCursorPos(mp);
			if MouseToAngle(Panel3D.ScreenToClient(mp),orbit,orbitangle) then begin
	            wunit2:=AddUnit(WorldUnit_Sel.FFileName,orbit,orbitangle);
				if wunit2<>nil then begin
                	...
                	if apo<>nil then begin
    	            	...
    	            	if zangle3<0 then zangle3:=360+zangle3;
		                ...
		                if zangle2<0 then zangle2:=360+zangle2;
    	                ...
    	                while apo<>nil do begin
        	    			if apo.FOwner.IndexOf(wunit2)>=0 then begin
        ...
        end else if (Msg.wParam in [VK_UP,VK_DOWN,VK_LEFT,VK_RIGHT]) then begin
    		zn:=1;
			if _shift or _ctrl then zn:=zn*5;

	    	if Msg.wParam=VK_UP then begin
    	    	ab_Camera_Pos.FOrbitAngle:=max(0,ab_Camera_Pos.FOrbitAngle-zn);
	    	end else if Msg.wParam=VK_DOWN then begin
...
//			Draw3D;
    	...
    	end else if (Msg.wParam in [VK_PRIOR,VK_NEXT]) then begin
        	if Msg.wParam=VK_PRIOR then zn:=-100
            else zn:=100;
        	ab_Camera_Radius:=ab_Camera_Radius+zn;

    	end else if (Msg.wParam in [VK_PRIOR,VK_HOME]) then begin
	        ab_Camera_Radius:=ab_WorldRadius+ab_Camera_RadiusDefaultFW;

    	end else if (Msg.wParam=integer('Z')) then begin
	        FormMain.MM_View_Zone.Checked:=true;

        	GetCursorPos(mp);
			if MouseToAngle(Panel3D.ScreenToClient(mp),orbit,orbitangle) then begin
            	if ZoneLink_Sel<>nil then ZoneLink_Sel.FSel:=false;
            	if Zone_Sel<>nil then Zone_Sel.FSel:=false;

            	apo:=Point_First;
                while apo<>nil do begin
                	if apo.FSel then apo.Sel:=false;
                	apo:=apo.FNext;
                end;
...
begin
	if (FMove) or (FMoveWorld) or (FActionType>0) then begin
    	FMove:=false;
        FMoveWorld:=false;
        FActionType:=0;
...
//    _shift:=(GetAsyncKeyState(VK_SHIFT) and $8000)=$8000;
	...
	if (ssRight in Shift) then begin
    	if (WorldUnit_Sel<>nil) and (PickUnit(Point(x,y))=WorldUnit_Sel) then begin
			if MouseToAngle(Point(x,y),FMoveOrbit,FMoveOrbitAngle) then begin
				FMove:=true;
				FMoveLP:=Point(x,y);
                FRotate:=false;
        ...
        end else if (Point_Sel<>nil) and (PickPoint(Point(x,y))<>nil) and (ab_InTop(PickPoint(Point(x,y)).FPosShow.z)) then begin
			if MouseToAngle(Point(x,y),FMoveOrbit,FMoveOrbitAngle) then begin
	        	FMove:=true;
				FMoveLP:=Point(x,y);
        ...
        end else if (Zone_Sel<>nil) and (PickZone(Point(x,y))<>nil) then begin
			if MouseToAngle(Point(x,y),FMoveOrbit,FMoveOrbitAngle) then begin
...
(*    end else if (ssLeft in Shift) and (not FMove) then begin
    ...
    if (not (FMove or FMoveWorld)) and (FActionType=0) and ((ssLeft in Shift)) and (not (ssRight in Shift)) then begin
		FSel:=true;
...
begin
	...
	if ((FMove) or (FMoveWorld) or (FActionType>0)) and (Button=mbRight) then begin
    	FMove:=false;
        FMoveWorld:=false;
        FActionType:=0;
		...
		Screen.Cursor:=crDefault;
		Mouse.Capture:=0;
    end else if FSel and (Button=mbLeft) then begin
        if FSelGraph<>nil then begin
        	if FSelRect.Left>FSelRect.Right then begin
            	zn:=FSelRect.Left; FSelRect.Left:=FSelRect.Right; FSelRect.Right:=zn;
            end;
        	if FSelRect.Top>FSelRect.Bottom then begin
            	zn:=FSelRect.Top; FSelRect.Top:=FSelRect.Bottom; FSelRect.Bottom:=zn;
            end;
    ...
    if (Button=mbLeft) and (not FMove) and (not selcancel) then begin
        zoneSel:=Zone_Sel;
        zlSel:=ZoneLink_Sel;
		wunitSel:=WorldUnit_Sel;
...
//			zlSel.UpdateGraph;
    	...
    	if _ctrl or _shift then begin
			zone:=PickZone(Point(x,y));

	        if (zoneSel<>nil) and (zone<>nil) and (zoneSel<>zone) and (ZoneLink_Find(zoneSel,zone)=nil) and (ZoneLink_Find(zone,zoneSel)=nil) then begin
				zl:=ZoneLink_Add;
    	        zl.FStart:=zoneSel;
        	    zl.FEnd:=zone;
        	...
        	obj:=zoneSel;
            if obj=nil then obj:=zlSel;
            if obj=nil then obj:=wunitSel;
            ...
            if li<>nil then begin
                ...
                if obj is TabZone then begin
                	zone:=obj as TabZone;
                    zone.FSel:=true;
                    zone.UpdateGraph;
                end else if obj is TabZoneLink then begin
                	zl:=obj as TabZoneLink;
                    zl.FSel:=true;
//                    zl.UpdateGraph;
                end else if obj is TabWorldUnit then begin
                	wunit:=obj as TabWorldUnit;
                    wunit.FSel:=true;
                    wunit.UpdateGraph;
                end else if obj is TPointAB then begin
                	apo:=obj as TPointAB;
                    apo.Sel:=true;
                end;
            end;
...
begin
    ...
    if hitok then LabelCoord.Caption:=Format('%.2f,%.2f',[orbit,orbitangle])
    else LabelCoord.Caption:='';

    if FMove and (hitok) and (WorldUnit_Sel<>nil) and (not (ssLeft in Shift)) and (not FRotate) then begin
		wunit:=WorldUnit_Sel;
        li:=Point_Find(wunit);

		ab_CalcAngleAndDist(FMoveOrbit,FMoveOrbitAngle,0,orbit,orbitangle,ab_WorldRadius,zang,zdist);
		if zang<0 then zang:=360+zang;
...
//            if apo.FParent=nil then begin
				...
				if zang2<0 then zang2:=360+zang2;
				ap:=ab_CalcNewPos(abPos(FMoveOrbit,FMoveOrbitAngle,zang),zang2,zdist2);
				with ab_CalcNewPos(ap,zdist) do begin
...
//        WorldUnit_CalcCenter;
    ...
    end else if FMove and (WorldUnit_Sel<>nil) and (ssLeft in Shift) then begin
    	FRotate:=true;
        ...
        for i:=0 to li.Count-1 do begin
        	apo:=li.Items[i];
            if apo.FOwner.Count<=1 then begin
				apo.FPos:=D3D_TransformVector(m,apo.FPos);
    	        ab_CalcAngle(apo.FPos,apo.FOrbit,apo.FOrbitAngle);
                apo.SetOrbit(apo.FOrbit,apo.FOrbitAngle);
//        	    apo.CalcPos;
...
//        WorldUnit_CalcCenter;
    ...
    end else if FMove and hitok then begin
		ab_CalcAngleAndDist(FMoveOrbit,FMoveOrbitAngle,0,orbit,orbitangle,ab_WorldRadius,zang,zdist);
		if zang<0 then zang:=360+zang;

        apo:=Point_First;
        while apo<>nil do begin
        	if not apo.FSel then begin apo:=apo.FNext; continue; end;
            if apo.FParent<>nil then begin apo:=apo.FNext; continue; end;
...
//        WorldUnit_CalcCenter;
    ...
    end else if FMoveWorld then begin
		ab_Camera_Pos.FOrbitAngle:=ab_Camera_Pos.FOrbitAngle+(FMoveLP.y-y)*0.2;
        if ab_Camera_Pos.FOrbitAngle<0 then ab_Camera_Pos.FOrbitAngle:=0
        else if ab_Camera_Pos.FOrbitAngle>180 then ab_Camera_Pos.FOrbitAngle:=180;

		ab_Camera_Pos.FOrbit:=AngleCorrect360(ab_Camera_Pos.FOrbit+(FMoveLP.x-x)*0.2);

    end else if FSel then begin
    	FSelRect.BottomRight:=Point(x,y);
        if (FSelGraph<>nil) or ((sqr(FSelRect.Left-FSelRect.Right)+sqr(FSelRect.Top-FSelRect.Bottom))>sqr(2)) then begin
		    SelGraphUpdate;
        end;

	end else if FActionType=1 then begin
		zone:=Zone_Sel;
    	if hitok then begin
        	zone.FOrb:=orbit;
			zone.FOrbAngle:=orbitangle;
	...
	end else if FActionType=2 then begin
		zone:=Zone_Sel;
    	if hitok then begin
...
var
	i:integer;
begin
	if FSWL=nil then Exit;
    for i:=0 to FSWL.Count-1 do begin
...
//    v3:=ab_TransformVector(ab_Camera_MatEnd,v3);
    ...
    dt:=0;
    while dt<(pi*2-0.001) do begin
        di:=distep*2;
        while di<(pi-distep*2-0.001) do begin
...
//	Edit1.Text:=IntToStr(integer(Application.Active));
	...
	dwzn:=timeGetTime;
	if (dwzn-FFPSLast)>1000 then begin
        LabelFPS.Caption:=IntToStr(FFPSCnt);
        FFPSLast:=dwzn;
        FFPSCnt:=0;
    end else inc(FFPSCnt);
...
//    ab_Camera_MatProj:=D3D_ProjectionMatrix(1,ab_WorldRadius*2.1,Angle360ToRad(ab_Camera_Fov),Panel3D.Width);
    ab_Camera_MatProj:=D3D_ProjectionMatrix(ab_Camera_Radius-ab_WorldRadius-100,ab_Camera_Radius+ab_WorldRadius+100,Angle360ToRad(ab_Camera_Fov),Panel3D.Width);
...
var
	orbit,orbitangle:single;
begin
	if not OpenDialogUnit.Execute then Exit;

	if MouseToAngle(Point(GSmeX,GSmeY),orbit,orbitangle) then begin
...
begin
    ...
    if (sou_cnt<1) or (des_cnt<1) then begin WorldUnit_Delete(des); Exit; end;

//    if (sou.PointCnt()>des.PointCnt()) then begin WorldUnit_Delete(des); Exit; end;

    cnt:=Max(sou_cnt,des_cnt);
    SetLength(sou_list,cnt); for i:=0 to cnt-1 do sou_list[i]:=nil;
    SetLength(des_list,cnt); for i:=0 to cnt-1 do des_list[i]:=nil;
    ...
    i:=0;
	sou_apo:=Point_First;
    while sou_apo<>nil do begin
    	if sou_apo.FOwner.IndexOf(sou)>=0 then begin
		    sou_list[i]:=sou_apo;
        	des_apo:=des.GetPoint(sou_apo.FId);
            if des_apo<>nil then begin
            	des_list[i]:=des_apo;
    ...
    des_apo:=Point_First;
    while des_apo<>nil do begin
    	if des_apo.FOwner.IndexOf(des)>=0 then begin
        	u:=0;
        	while u<i do begin
            	if des_list[u]=des_apo then break;
                inc(u);
            end;
            if u>=i then begin
            	des_list[i]:=des_apo;
    ...
    for i:=0 to cnt-1 do begin
    	if des_list[i]<>nil then begin
		    des_orb[i]:=des_list[i].FOrbit;
		    des_orbAngle[i]:=des_list[i].FOrbitAngle;
        end;
    	if (sou_list[i]<>nil) and (des_list[i]<>nil) then begin
...
//        	des_list[i].FRadius:=sou_list[i].FRadius;
    ...
    if sou_cnt<des_cnt then begin
        ...
        if sou_angle_rel<0 then sou_angle_rel:=360+sou_angle_rel;
        ...
        if des_angle_rel<0 then des_angle_rel:=360+des_angle_rel;
	    ...
	    for i:=0 to cnt-1 do begin
    		if (sou_list[i]=nil) and (des_list[i]<>nil) then begin
	    	    ab_CalcAngleAndDist(des_orb_center,des_orbangle_center,0,
                                    ...
                                    des_angle_p,des_r_p);
		        if des_angle_p<0 then des_angle_p:=360+des_angle_p;
...
begin
    ...
    while atr<>nil do begin
        ...
        begin
        	if t>0 then begin
            	if ab_InTop(atr.FV[0].FVer.FPosShow.z) and
                   ab_InTop(atr.FV[1].FVer.FPosShow.z) and
...
begin
    ...
    while ali<>nil do begin
    	if ali.Hit(mp) then begin
           	if ab_InTop(ali.FVerStart.FPosShow.z) and ab_InTop(ali.FVerEnd.FPosShow.z) then begin
   	    	 	Result:=ali;
    	        Exit;
            end;
        end;
...
begin
	atr:=PickTriangle(mp);
    if (atr=nil) or (atr.FOwner=nil) or (not (atr.FOwner is TabWorldUnit)) then begin
    	ali:=PickLine(mp);
        if (ali<>nil) and (ali.FOwner<>nil) and (ali.FOwner is TabWorldUnit) then begin
...
begin
	...
	apo:=Point_First;
    while apo<>nil do begin
    	if ab_InTop(apo.FPosShow.z) then begin
	    	if apo.FParent=nil then begin
		    	td:=sqr(apo.FPosShow.x-mp.x)+sqr(apo.FPosShow.y-mp.y);

    	    	if (td<sqr(5)) and (td<md) then begin
        			md:=td;
	            	Result:=apo;
		        end;
        	end;
...
var
    ...
    md,td:single;
begin
	if not FormMain.MM_View_Zone.Checked then begin Result:=nil; exit; end;
    ...
    while zone<>nil do begin
    	if ab_InTop(zone.FPosShow.z) then begin
...
begin
    ...
    while zl<>nil do begin
    	...
    	if (not ab_InTop(zl.FStart.FPosShow.z)) or (not ab_InTop(zl.FEnd.FPosShow.z)) then begin zl:=zl.FNext; continue; end;
    	...
    	if (t<0) or (t>1.0) then begin zl:=zl.FNext; continue; end;

		r:=(-l2.y)*(l1.x)-(-l2.x)*(l1.y);
    	r:=(r/l)*sqrt(l);

        if abs(r)>3 then begin zl:=zl.FNext; continue; end;
...
begin
    ...
    zone:=PickZone(mp);
    if zone<>nil then li.Add(zone);

	zl:=PickZoneLink(mp);
    if zl<>nil then li.Add(zl);

	wunit:=PickUnit(mp);
    if wunit<>nil then li.Add(wunit);

	apo:=PickPoint(mp);
    if apo<>nil then li.Add(apo);
...
begin
    ...
    if MM_View_WorldLine.Checked then begin
	    SWLBuild;
    end else begin
	    SWLClear;
    end;
...
var
	bd:TBufEC;
begin
	if (GFileName='') or (Sender<>nil) then begin
...
begin
    ...
    try
        ...
        if bd.GetDWORD()<>$57424152 then EError('Incorrect format');
		GLoadVersion:=bd.GetInteger;
        if GLoadVersion>GVersion then EError('Incorrect version');
    ...
    if MM_View_WorldLine.Checked then SWLBuild;
    UpdateCaption;
    UpdateInfo;
...
begin
	if (not FileExists(GRangersPath+'\CFG\CD\ABMap.txt')) or
       (not FileExists(GRangersPath+'\CFG\ABMap.txt')) then
    begin
...
begin
    ...
    while wunit<>nil do begin
    	if wunit.FFileName=tstr then li.Add(wunit);
		wunit:=wunit.FNext;
    end;
...
procedure TFormMain.EditMainWorldRadiusKeyDown(Sender: TObject;
