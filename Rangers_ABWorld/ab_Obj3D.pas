...
procedure ab_Obj3D_Draw(ln: integer);

var
  ab_Obj3D_First: TabObj3D = nil;
  ab_Obj3D_Last: TabObj3D = nil;

  ab_Obj3D_Texture: boolean = false;
...
begin
  if FVer = nil then
    if GR_lpDev.CreateVertexBuffer(len * sizeof(TabVer3D), 0, D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, FVer) <> D3D_OK then
      EError('TabObj3D.CreateVertexBuffer');
  FVerCnt := len;
end;
...
begin
  if FVerOpen = 0 then
    if FVer.Lock(0, FVerCnt * sizeof(TabVer3D), FVerBuf, 0) <> D3D_OK then
      EError('TabObj3D.Lock');
  Inc(FVerOpen);
end;
...
begin
  ...
  if FUnitLast <> nil then
    FUnitLast.FNext := el;
  el.FPrev := FUnitLast;
  el.FNext := nil;
  FUnitLast := el;
  if FUnitFirst = nil then
    FUnitFirst := el;
...
begin
  ...
  if FUnitLast = el then
    FUnitLast := el.FPrev;
  if FUnitFirst = el then
    FUnitFirst := el.FNext;
...
begin
  ...
  if GR_lpDev.CreateIndexBuffer(cnt * sizeof(word), D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_MANAGED, FIndex) <>
    D3D_OK then
    EError('TabObj3D CreateIndexBuffer');
  if FIndex.Lock(0, cnt * sizeof(word), ib, 0) <> D3D_OK then
    EError('TabObj3D Index.Look');
...
begin
  ...
  begin
    ...
    else if (un.FTex = nil) or (un.FTex.FPath <> tstr) then
    begin
      if un.FTex <> nil then
      begin
        ab_Tex_Free(un.FTex);
...
{    D3DLVERTEX* pVertices    = new D3DLVERTEX[dwNumVertices];
  ...
  for y := 0 to dwNumRings - 1 do
  begin
    y0 := cos(fDAngY0);
    r0 := sin(fDAngY0);
    tv := (1.0 - y0) / 2;

    for x := 0 to (dwNumRings * 2) + 1 - 1 do
    begin
      fDAngX0 := x * fDAng;
  ...
  for y := 0 to dwNumRings - 1 - 1 do
    for x := 0 to (dwNumRings * 2) - 1 do
    begin
      un.FVer[index] := ((y + 0) * (dwNumRings * 2 + 1) + (x + 0));
      Inc(index);
      un.FVer[index] := ((y + 1) * (dwNumRings * 2 + 1) + (x + 0));
      Inc(index);
      un.FVer[index] := ((y + 0) * (dwNumRings * 2 + 1) + (x + 1));
      Inc(index);
      un.FVer[index] := ((y + 0) * (dwNumRings * 2 + 1) + (x + 1));
      Inc(index);
      un.FVer[index] := ((y + 1) * (dwNumRings * 2 + 1) + (x + 0));
      Inc(index);
      un.FVer[index] := ((y + 1) * (dwNumRings * 2 + 1) + (x + 1));
      Inc(index);
    end;
...
begin
  ...
  begin
    ...
    if un.FVer[index] >= segcnt then
      un.FVer[index] := 0;
    Inc(index);
    un.FVer[index] := i + 1 + segcnt;
    if un.FVer[index] >= (segcnt + segcnt) then
      un.FVer[index] := segcnt;
    Inc(index);
    un.FVer[index] := i + segcnt;
    Inc(index);
  end;
...
var
  ...
  m: D3DMATRIX;
begin
  if (FVerCnt < 6) or ((FVerCnt and 1) = 1) then
    exit;
...
begin
  if FVerCnt <= 0 then
    exit;
  if (FPosOld.x = pos.x) and (FPosOld.y = pos.y) and (FPosOld.z = pos.z) then
    exit;
  VerOpen;
  for i := 0 to FVerCnt - 1 do
  begin
    rp := Ver(i);
...
var
  un: PabObj3Dunit;
begin
  if FIndexRebuild then
    IndexBuild;

  if (FVer = nil) or (FIndex = nil) then
    exit;

  if GR_lpDev.SetStreamSource(0, FVer, sizeof(TabVer3D)) <> D3D_OK then
    EError('TabObj3D Draw');
  if GR_lpDev.SetIndices(FIndex, 0) <> D3D_OK then
    EError('TabObj3D Draw');
  if GR_lpDev.SetVertexShader(D3DFVF_CUSTOMVERTEX) <> D3D_OK then
    EError('TabObj3D Draw');
...
procedure ab_Obj3D_Clear();
begin
  while ab_Obj3D_First <> nil do
    ab_Obj3D_Delete(ab_Obj3D_Last);
end;
...
begin
  ...
  if ab_Obj3D_Last <> nil then
    ab_Obj3D_Last.FNext := el;
  el.FPrev := ab_Obj3D_Last;
  el.FNext := nil;
  ab_Obj3D_Last := el;
  if ab_Obj3D_First = nil then
    ab_Obj3D_First := el;
...
begin
  ...
  if el.FNext <> nil then
    el.FNext.FPrev := el.FPrev;
  if ab_Obj3D_Last = el then
    ab_Obj3D_Last := el.FPrev;
  if ab_Obj3D_First = el then
    ab_Obj3D_First := el.FNext;
...
begin
  ...
  while el <> nil do
  begin
    if (el.FDrawFromObjLoop) and (el.FLoopDraw = ln) then
      el.Draw;
    el := el.FNext;
  end;
end;
