...
function SmeToLineAndChar(tstr: WideString; tsme: integer; enteradd: integer; var tline: integer;
  var tchar: integer): boolean;
...
type
  ...
  //          = 2 - file not found
  //          = 3 - error
  FunctionIncludeUnitEC = function(in_unit: integer; in_unitname: WideString; in_canrepeat: boolean;
    var out_unit: integer; out_ca: TCodeAnalyzerEC): integer;
  ...
  public
    ...
    function BuildStr(strshowquate: boolean = false; ustart: TCodeAnalyzerUnitEC = nil;
      uend: TCodeAnalyzerUnitEC = nil): WideString;
  ...
  eieTypeEC = (eieMinus, eieAdd, eieSub, eieMul, eieDiv, eieMod, eieBitAnd, eieBitOr, eieBitXor, eieBitNot,
    eieAnd, eieOr, eieNot, eieShl, eieShr, eieLess, eieMore, eieEqual, eieNotEqual, eieLessEqual, eieMoreEqual,
    eieAssume, eieFun, eieArray);
  ...
  public
    ...
    function Compiler(ca: TCodeAnalyzerEC; curclass: TCodeEC = nil; castart: TCodeAnalyzerUnitEC = nil;
      caend: TCodeAnalyzerUnitEC = nil; endcompl: PDWORD = nil): WideString;
  ...
  public
    ...
    function Compiler(ca: TCodeAnalyzerEC; codeunit: integer; funinclude: FunctionIncludeUnitEC = nil;
      el: TCodeAnalyzerUnitEC = nil; elendcomp: PDWORD = nil; curclass: TCodeEC = nil): WideString;
    function CompilerR(ca: TCodeAnalyzerEC; codeunit: integer; funinclude: FunctionIncludeUnitEC;
      el: TCodeAnalyzerUnitEC; insertto: TCodeUnitEC; elendcomp: PDWORD; elcontinue: PDWORD;
      opBreak: TCodeUnitEC; opContinue: TCodeUnitEC; curclass: TCodeEC): WideString;
    procedure Link(va: TVarArrayEC);
    procedure LinkAll(va: TVarArrayEC);
    procedure LinkClass;
    procedure UnLink(va: TVarArrayEC);
...
type
  cutTypeEC = (cutConstInt, cutConstDW, cutConstFloat, cutConstStr, cutOperator, cutOperatorUnary,
    cutOpen, cutClose, cutOpenArray, cutCloseArray, cutName, cutFun, cutArray, cutVar, cutComma, cutAssume);
...
begin
  ...
  begin
    if no = tsme then
      exit;
    if (tstr[i + 1] = #13) or (tstr[i + 1] = #10) then
    begin
      Inc(tline);
      tchar := 0;
      no := no + enteradd;
      if (tstr[i + 1 + 1] = #13) or (tstr[i + 1 + 1] = #10) then
      begin
        Inc(i);
        Inc(no);
      end;
...
begin
  len := Length(tstr);
  for i := 0 to len - 1 do
    if ((tstr[i + 1] < '0') or (tstr[i + 1] > '9')) and ((tstr[i + 1] <> '-') or (i > 0)) then
    begin
      result := false;
      exit;
    end;
...
begin
  ...
  if (el.FType <> caeName) or (not OnlyIntEXP(el.FStr)) then
    exit;
  zn := StrToIntEXP(el.FStr);
  el := el.FNext;
  if el = nil then
    exit;
  if el.FType <> caePoint then
    exit;
  el := el.FNext;
  if el = nil then
    exit;
  if (el.FType <> caeName) then
    exit;
  ...
  begin
    ...
    if (ch >= '0') and (ch <= '9') then
      tzn := tzn * 10 + (integer(ch) - integer('0'))
  ...
  begin
    el := el.FNext;
    if (el = nil) or (el.FNext = nil) or (el.FNext.FType <> caeName) or (not OnlyIntEXP(el.FNext.FStr)) then
      exit;
    if (el.FType = caeSub) then
      expzn := -StrToIntEXP(el.FNext.FStr)
    else if (el.FType = caeAdd) then
      expzn := StrToIntEXP(el.FNext.FStr)
    else
      exit;
    el := el.FNext.FNext;
  ...
  if expzn > 0 then
    zn := zn * power(10, expzn)
  else if expzn < 0 then
    zn := zn / power(10, -expzn);
...
begin
  ...
  begin
    ...
    begin
      ch := el.FStr[i + 1];
      if (ch >= '0') and (ch <= '9') then
        zn := zn * 16 + (DWORD(ch) - DWORD('0'))
      else if (ch >= 'a') and (ch <= 'f') then
        zn := zn * 16 + (10 + DWORD(ch) - DWORD('a'))
      else if (ch >= 'A') and (ch <= 'F') then
        zn := zn * 16 + (10 + DWORD(ch) - DWORD('A'))
      else
        exit;
...
begin
  zn := '';
  while (elt.FType = caeName) and IsVarEXP(elt.FStr) do
  begin
    zn := zn + elt.FStr;
    elt := elt.FNext;
    if (elt = nil) or (elt.FType <> caePoint) then
      break;
    zn := zn + '.';
    elt := elt.FNext;
    if (elt = nil) or (not ((elt.FType = caeName) and IsVarEXP(elt.FStr))) then
    begin
      zn := '';
      break;
    end;
...
begin
  ...
  else if zn = vtStr then
  begin
    if FVType = vtUnknown then
      FVStr := ''
    else if FVType = vtInt then
      FVStr := IntToStr(FVInt)
    else if FVType = vtDW then
      FVStr := IntToStr(FVDW)
    else if FVType = vtFloat then
      FVStr := FloatToStrEXP(FVFloat)
    else if FVType = vtStr then
    else if FVType = vtExternFun then
      FVStr := ''
    else if FVType = vtLibraryFun then
...
{    if VType=vtRef then begin // error ???
  ...
  if FVType = vtRef then
  begin // naverno nado tak
    if FVRef <> nil then
      FVRef.SetVType(zn);
    exit;
  end;
...
begin
  tv := GetByRef;
  if tv = nil then
    raise ExceptionExpressionEC.Create('Undefined ref. Name=' + FName);
  result := tv.FVType;
end;
...
begin
  ...
  FVLibraryFun := nil;
  if sou.FVLibraryFun <> nil then
  begin
    SetLength(FVLibraryFun, High(sou.FVLibraryFun) + 1);
    for i := 0 to High(FVLibraryFun) do
      FVLibraryFun[i] := sou.FVLibraryFun[i];
  end;
  FVFun := nil;
  if sou.FVFun <> nil then
  begin
    FVFun := TCodeEC.Create;
...
begin
  if FVType = vtUnknown then
    ...
    result := Trunc(FVFloat)
  else if FVType = vtStr then
    result := StrToIntEXP(FVStr)
  else if FVType = vtExternFun then
    result := 0
  else if FVType = vtLibraryFun then
    result := 0
...
begin
  if FVType = vtUnknown then
    ...
    result := IntToStr(FVInt)
  else if FVType = vtDW then
    result := IntToStr(FVDW)
  else if FVType = vtFloat then
    result := FloatToStrEXP(FVFloat)
  else if FVType = vtStr then
    result := FVStr
  else if FVType = vtExternFun then
    result := ''
...
begin
  ...
  begin
    ...
    FVInt := StrToIntEXP(zn)
  else if FVType = vtDW then
    FVDW := StrToIntEXP(zn)
  else if FVType = vtFloat then
    FVFloat := StrToFloatEXP(zn)
  else if FVType = vtStr then
    FVStr := zn
  else if FVType = vtExternFun then
    FVExternFun := nil
...
begin
  result := self;
  while (result <> nil) and (result.FVType = vtRef) do
    result := result.FVRef;
end;
...
begin
  ...
  begin
    tvar := va.Items[i];
    if (tvar.FVType = vtArray) and (tvar.VArray <> nil) then
      ArrayClearREC(tvar.VArray);
  end;
  va.Free;
end;
...
begin
  if (VType <> vtArray) or (VArray = nil) then
    exit;
...
begin
  ...
  if des.FVType = vtUnknown then
  else if des.FVType = vtInt then
    des.VInt := sou.VInt
  else if des.FVType = vtDW then
    des.VDW := sou.VDW
  else if des.FVType = vtFloat then
    des.VFloat := sou.VFloat
  else if des.FVType = vtStr then
    des.VStr := sou.VStr
  else if des.FVType = vtExternFun then
    des.VExternFun := sou.VExternFun
  else if des.FVType = vtLibraryFun then
  begin
    des.FVLibraryFun := nil;
    if sou.FVLibraryFun <> nil then
    begin
      SetLength(des.FVLibraryFun, High(sou.FVLibraryFun) + 1);
      for i := 0 to High(des.FVLibraryFun) do
        des.FVLibraryFun[i] := sou.FVLibraryFun[i];
    end;
  end
  else if des.FVType = vtFun then
    des.VFun := sou.VFun
  else if des.FVType = vtClass then
    des.VClass := sou.VClass
  else if des.FVType = vtArray then
    des.VArray := sou.VArray
  else
...
begin
  case VType of
    ...
    vtClass: result := VClass <> sou.VClass;
    vtArray: result := false;
    else raise ExceptionExpressionEC.Create('NotEqual');
  end;
end;
...
begin
  case VType of
    ...
    vtClass: result := false;
    vtArray: result := false;
    else raise ExceptionExpressionEC.Create('LessEqual');
  end;
end;
...
begin
  case VType of
    ...
    vtClass: result := false;
    vtArray: result := false;
    else raise ExceptionExpressionEC.Create('MoreEqual');
  end;
end;
...
begin
  ...
  if result = nil then
    raise ExceptionExpressionEC.Create('Var not found:' + tname);
end;
...
begin
  if FItem = nil then
    FItem := HeapAlloc(GetProcessHeap(), 0, (FCount + 1) * 4)
  else
    FItem := HeapRealloc(GetProcessHeap(), 0, FItem, (FCount + 1) * 4);
  Items[FCount] := el;
...
begin
  ...
  if FItem = nil then
    ...
    FItem := HeapRealloc(GetProcessHeap(), 0, FItem, FCount * 4);
  for i := FCount - 1 to tono + 1 do
    Items[i] := Items[i - 1];
  Items[tono] := el;
...
begin
  ...
  if (FLastFree = el) then
    FLastFree := el.FPrev;
  if (FFirstFree = el) then
    FFirstFree := el.FNext;
...
begin
  ...
  begin
    ...
    begin
      if sim = '(' then
      begin // (
        if (zsme >= 0) and (zlen > 0) then
        begin
          FLast.FStr := FLast.FStr + Copy(wstr, zsme + 1, zlen);
          Inc(FLast.FLen, zlen);
        end;
        zsme := -1;
        tu := UnitAdd();
      ...
      else if (sim = '/') and ((i + 1) < Count) and (wstr[i + 1 + 1] = '*') then
      begin // /*
        if (zsme >= 0) and (zlen > 0) then
        begin
          FLast.FStr := FLast.FStr + Copy(wstr, zsme + 1, zlen);
      ...
      begin // space
        ...
        zsme := -1;
        if (FLast = nil) or ((FLast <> nil) and (FLast.FType <> caeSpace)) then
        begin
          tu := UnitAdd();
          tu.FType := caeSpace;
          tu.FSme := i + startstr;
      ...
      begin // Enter
        ...
        //        numchar:=-1;
        //        inc(startstr);
        if ((i + 1) < Count) and ((wstr[i + 1 + 1] = #13) or (wstr[i + 1 + 1] = #10)) then
        begin
          Inc(i);
          Inc(tu.FLen);
        end;
      end
      else if (sim = '"') or (sim = '''') then
      begin  // String
        if (zsme >= 0) and (zlen > 0) then
        begin
          FLast.FStr := FLast.FStr + Copy(wstr, zsme + 1, zlen);
        ...
        if (FLast = nil) or (FLast.FType <> caeName) then
        begin
          tu := UnitAdd();
          ...
          Inc(zlen)//                    FLast.FStr:=FLast.FStr+sim; inc(FLast.FLen);
        // Name
      ;
    end
    else if FLast.FType = caeStr then
    begin
      //                inc(FLast.FLen);
      Inc(zlen);
      if sim = '\' then
      begin
        if ((i + 1) < Count) and (wstr[i + 1 + 1] = '\') then
        begin
          if (zsme >= 0) and (zlen > 0) then
          begin
            FLast.FStr := FLast.FStr + Copy(wstr, zsme + 1, zlen - 1);
            Inc(FLast.FLen, zlen - 1);
          end;
        ...
        else if ((i + 1) < Count) and (wstr[i + 1 + 1] = '"') then
        begin
          if (zsme >= 0) and (zlen > 0) then
          begin
            FLast.FStr := FLast.FStr + Copy(wstr, zsme + 1, zlen - 1);
        ...
        else if ((i + 1) < Count) and (wstr[i + 1 + 1] = 'n') then
        begin
          if (zsme >= 0) and (zlen > 0) then
          begin
            FLast.FStr := FLast.FStr + Copy(wstr, zsme + 1, zlen - 1);
      ...
      else //                    FLast.FStr:=FLast.FStr+sim;
        //                    inc(zlen);
      ;
    end;
    Inc(i);
...
begin
  ...
  tstr := '';
  tu := ustart;
  while (tu <> nil) and (tu <> uend) do
  begin

    //tstr:=tstr+'~';

    if tu.FType = caeEnter then
      tstr := tstr + #13#10
    else if (tu.FType = caeSpace) then
      tstr := tstr + ' '
    else if (tu.FType = caeOpen1) then
      tstr := tstr + '('
    else if (tu.FType = caeClose1) then
      tstr := tstr + ')'
    else if (tu.FType = caeOpen2) then
      tstr := tstr + '{'
    else if (tu.FType = caeClose2) then
      tstr := tstr + '}'
    else if (tu.FType = caeOpen3) then
      tstr := tstr + '['
    else if (tu.FType = caeClose3) then
      tstr := tstr + ']'
    else if (tu.FType = caeOpen4) then
      tstr := tstr + '/*'
    else if (tu.FType = caeClose4) then
      tstr := tstr + '*/'
    else if (tu.FType = caeCom) then
      tstr := tstr + '//'
    else if (tu.FType = caePoint) then
      tstr := tstr + '.'
    else if (tu.FType = caePointer) then
      tstr := tstr + '->'
    else if (tu.FType = caeAdd) then
      tstr := tstr + '+'
    else if (tu.FType = caeSub) then
      tstr := tstr + '-'
    else if (tu.FType = caeMul) then
      tstr := tstr + '*'
    else if (tu.FType = caeDiv) then
      tstr := tstr + '/'
    else if (tu.FType = caeMod) then
      tstr := tstr + '%'
    else if (tu.FType = caeBitAnd) then
      tstr := tstr + '&'
    else if (tu.FType = caeBitOr) then
      tstr := tstr + '|'
    else if (tu.FType = caeBitXor) then
      tstr := tstr + '^'
    else if (tu.FType = caeBitNot) then
      tstr := tstr + '~'
    else if (tu.FType = caeAnd) then
      tstr := tstr + '&&'
    else if (tu.FType = caeOr) then
      tstr := tstr + '||'
    else if (tu.FType = caeNot) then
      tstr := tstr + '!'
    else if (tu.FType = caeShl) then
      tstr := tstr + '<<'
    else if (tu.FType = caeShr) then
      tstr := tstr + '>>'
    else if (tu.FType = caeAssume) then
      tstr := tstr + '='
    else if (tu.FType = caeEqual) then
      tstr := tstr + '=='
    else if (tu.FType = caeNotEqual) then
      tstr := tstr + '!='
    else if (tu.FType = caeLess) then
      tstr := tstr + '<'
    else if (tu.FType = caeMore) then
      tstr := tstr + '>'
    else if (tu.FType = caeLessEqual) then
      tstr := tstr + '<='
    else if (tu.FType = caeMoreEqual) then
      tstr := tstr + '>='
    else if (tu.FType = caeSemicolon) then
      tstr := tstr + ';'
    else if (tu.FType = caeColon) then
      tstr := tstr + ':'
    else if (tu.FType = caeComma) then
      tstr := tstr + ','
    else if (tu.FType = caeStr) and strshowquate then
      tstr := tstr + '"' + tu.FStr + '"'
    else if (tu.FType = caeStr) then
      tstr := tstr + tu.FStr
    else if (tu.FType = caeName) then
      tstr := tstr + tu.FStr;
...
begin
  ...
  while tu <> nil do
  begin
    if (tu.FType = caeOpen1) or (tu.FType = caeOpen2) or (tu.FType = caeOpen3) or (tu.FType = caeOpen4) then
      Inc(Count);
    if (tu.FType = caeClose1) or (tu.FType = caeClose2) or (tu.FType = caeClose3) or (tu.FType = caeClose4) then
      Inc(count2);
    tu := tu.FNext;
  end;
...
begin
  ...
  begin
    ...
    if (fcomLine = false) and (fcomBlock = 0) then
    begin
      if tu2.FType = caeOpen4 then
      begin
        fcomBlock := 1;
        UnitDel(tu2);
      end
      else if tu2.FType = caeCom then
      begin
        fcomLine := true;
        UnitDel(tu2);
      end;
    ...
    begin
      if (fcomBlock <> 0) and (tu2.FType = caeOpen4) then
        Inc(fcomBlock)
      else if fcomLine and (tu2.FType = caeEnter) then
        fcomLine := false
      else if (fcomBlock <> 0) and (tu2.FType = caeClose4) then
        Dec(fcomBlock);
      UnitDel(tu2);
    end;
  end;
...
begin
  if zn = caeAdd then
    FType := eieAdd
  else if zn = caeSub then
    FType := eieSub
  else if zn = caeMul then
    FType := eieMul
  else if zn = caeDiv then
    FType := eieDiv
  else if zn = caeMod then
    FType := eieMod
  else if zn = caeBitAnd then
    FType := eieBitAnd
  else if zn = caeBitOr then
    FType := eieBitOr
  else if zn = caeBitXor then
    FType := eieBitXor
  else if zn = caeBitNot then
    FType := eieBitNot
  else if zn = caeAnd then
    FType := eieAnd
  else if zn = caeOr then
    FType := eieOr
  else if zn = caeNot then
    FType := eieNot
  else if zn = caeShl then
    FType := eieShl
  else if zn = caeShr then
    FType := eieShr
  else if zn = caeEqual then
    FType := eieEqual
  else if zn = caeNotEqual then
    FType := eieNotEqual
  else if zn = caeLess then
    FType := eieLess
  else if zn = caeMore then
    FType := eieMore
  else if zn = caeLessEqual then
    FType := eieLessEqual
  else if zn = caeMoreEqual then
    FType := eieMoreEqual
  else
...
begin
  ...
  FCount := sou.FCount;
  FIndex := nil;
  if sou.FIndex <> nil then
  begin
    SetLength(FIndex, FCount);
    for i := 0 to FCount - 1 do
      FIndex[i] := sou.FIndex[i];
  end;
end;

destructor TExpressionVarEC.Destroy;
begin
  if FType = evteTemp then
    FVar.Free;
  FVar := nil;
  FPath := nil;
  inherited Destroy;
...
begin
  ...
  FType := sou.FType;
  FVar := nil;
  if (FType = evteExtern) then
    FVar := sou.FVar
  else if (FType = evteTemp) and (sou.FVar <> nil) then
  begin
    FVar := TVarEC.Create;
    FVar.CopyFrom(sou.FVar);
  end;
  if sou.FPath <> nil then
  begin
...
begin
  ...
  cnt := 1;
  for i := (iend + 1) to lenstr - 1 do
    if str[i + 1] = '.' then
      Inc(cnt);
  SetLength(FPath, cnt);

  i := 0;
  while (iend + 1) < lenstr do
  begin
    istart := iend + 1;
    iend := istart;
    while iend < lenstr do
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if result.VType = vtClass then
          result := result.VClass.FindClassMember(FPath[i])
        else if result.VType = vtFun then
...
begin
  while FVarCount > 0 do
    VarDel(FVarCount - 1);
  if not FInstrExtern then
    while FInstrCount > 0 do
      InstrDel(FInstrCount - 1);
  FRet := -1;
  FInstrExtern := false;
end;
...
begin
  ...
  for i := 0 to sou.FVarCount - 1 do
    EVar[VarAdd()].CopyFrom(sou.EVar[i]);

  for i := 0 to sou.FInstrCount - 1 do
...
begin
  Inc(FVarCount);
  if FVar = nil then
    FVar := HeapAlloc(GetProcessHeap(), 0, FVarCount * 4)
  else
    FVar := HeapRealloc(GetProcessHeap(), 0, FVar, FVarCount * 4);
...
var
  i: integer;
begin
  if (no < 0) or (no >= FVarCount) then
    exit;
  EVar[no].Free;
  for i := no to FVarCount - 2 do
    EVar[i] := EVar[i + 1];
  Dec(FVarCount);
  if FVarCount <= 0 then
  begin
...
begin
  Inc(FInstrCount);
  if FInstr = nil then
    FInstr := HeapAlloc(GetProcessHeap(), 0, FInstrCount * 4)
  else
    FInstr := HeapRealloc(GetProcessHeap(), 0, FInstr, FInstrCount * 4);
...
var
  i: integer;
begin
  if (no < 0) or (no >= FInstrCount) then
    exit;
  Instr[no].Free;
  for i := no to FInstrCount - 2 do
    Instr[i] := Instr[i + 1];
  Dec(FInstrCount);
  if FInstrCount <= 0 then
  begin
...
{$WARNINGS OFF}
function TExpressionEC.Compiler(ca: TCodeAnalyzerEC; curclass: TCodeEC; castart: TCodeAnalyzerUnitEC;
  caend: TCodeAnalyzerUnitEC; endcompl: PDWORD): WideString;
var
  el, el2: TCodeAnalyzerUnitEC;
...
begin
  ...
  begin
    ...
    if el.FType = caeOpen4 then //                fcomBlock:=true;
    else if el.FType = caeCom then //                fcomLine:=true;
    else if el.FType = caeStr then
    begin
      cpu := cp.UnitAdd;
      cpu.FSme := el.FSme;
      cpu.FLen := el.FLen;
    ...
    else if el.FType = caeComma then
    begin
      if fopen <= 0 then
        break;
      cpu := cp.UnitAdd;
    ...
    else if el.FType = caeAssume then
    begin
      cpu := cp.UnitAdd;
      cpu.FSme := el.FSme;
      cpu.FLen := el.FLen;
    ...
    else if el.FType = caeOpen1 then
    begin
      cpu := cp.UnitAdd;
      cpu.FSme := el.FSme;
      cpu.FLen := el.FLen;
    ...
    else if el.FType = caeClose1 then
    begin
      Dec(fopen);
      if fopen < 0 then
        break;
    ...
    else if el.FType = caeOpen3 then
    begin
      cpu := cp.UnitAdd;
      cpu.FSme := el.FSme;
      cpu.FLen := el.FLen;
    ...
    else if el.FType = caeClose3 then
    begin
      Dec(fopen);
      if fopen < 0 then
        break;
    ...
    else if (el.FType = caeBitNot) or (el.FType = caeNot) then
    begin
      cpu := cp.UnitAdd;
      cpu.FSme := el.FSme;
      cpu.FLen := el.FLen;
    ...
    else if (el.FType = caeAdd) or (el.FType = caeSub) or (el.FType = caeMul) or
      (el.FType = caeDiv) or (el.FType = caeMod) or
      (el.FType = caeSub) or (el.FType = caeBitAnd) or
      (el.FType = caeBitOr) or (el.FType = caeBitXor) or
      (el.FType = caeAnd) or (el.FType = caeOr) or
      (el.FType = caeShl) or (el.FType = caeShr) or
      (el.FType = caeEqual) or (el.FType = caeNotEqual) or (el.FType = caeLess) or
      (el.FType = caeMore) or (el.FType = caeLessEqual) or (el.FType = caeMoreEqual) then
    begin
      cpu := cp.UnitAdd;
      cpu.FSme := el.FSme;
      cpu.FLen := el.FLen;
    ...
    else if el.FType = caeName then
    begin
      el2 := el;
      if DecodeFloatEXP(el2, zfloat) then
      begin
        cpu := cp.UnitAdd;
        cpu.FSme := el.FSme;
        if el2 = nil then
          cpu.FLen := ca.FLast.FSme + ca.FLast.FLen - el.FSme
        else
          cpu.FLen := el2.FPrev.FSme + el2.FPrev.FLen - el.FSme;
        cpu.FType := cutConstFloat;
        cpu.FVFloat := zfloat;
        el := el2;
        continue;
      end
      else if DecodeDWEXP(el2, zdw) then
      begin
        cpu := cp.UnitAdd;
        cpu.FSme := el.FSme;
        if el2 = nil then
      ...
      else if DecodeIntEXP(el2, zint) then
      begin
        cpu := cp.UnitAdd;
        cpu.FSme := el.FSme;
        if el2 = nil then
      ...
      else if DecodeStrEXP(el2, zstr) then
      begin
        cpu := cp.UnitAdd;
        cpu.FSme := el.FSme;
        if el2 = nil then
      ...
      else if DecodeVarEXP(el2, zstr) then
      begin
        cpu := cp.UnitAdd;
        cpu.FSme := el.FSme;
        if el2 = nil then
...
{                end else if IsVarEXP(el.FStr) then begin
    ...
    else if (el.FType = caeSemicolon) then
      break
    else if (el.FType = caeEnter) or (el.FType = caeSpace) then
    else
    begin
      result := CErrorEXP(0, el.FSme);
      cp.Free;
  ...
  while cpu <> nil do
  begin
    if (cpu.FType = cutOpen) and (cpu.FPrev <> nil) and (cpu.FPrev.FType = cutName) then
    begin
      cpu.FType := cutFun;
    ...
    else if (cpu.FType = cutOpenArray) then
    begin
      if (cpu.FPrev <> nil) and (cpu.FPrev.FType = cutName) then
      begin
        cpu.FType := cutArray;
        cpu.FStr := cpu.FPrev.FStr;
        cp.UnitDel(cpu.FPrev);
    ...
    else if cpu.FType = cutOpen then
      Inc(fopen)
    else if cpu.FType = cutClose then
      Dec(fopen)
    else if cpu.FType = cutCloseArray then
      Dec(fopen);
    cpu := cpu.FNext;
  end;
  ...
  while cpu <> nil do
  begin
    if (cpu.FType = cutOperator) and (cpu.FOper = caeSub) and (cpu.FNext <> nil) and
      ((cpu.FNext.FType = cutConstInt) or (cpu.FNext.FType = cutConstFloat)) then
    begin
      if (cpu.FPrev = nil) or ((cpu.FPrev.FType <> cutConstInt) and (cpu.FPrev.FType <> cutConstDW) and
        (cpu.FPrev.FType <> cutConstFloat) and (cpu.FPrev.FType <> cutClose) and (cpu.FPrev.FType <> cutCloseArray) and
        (cpu.FPrev.FType <> cutName)) then
      begin
        cpu := cpu.FNext;
    ...
    else if (cpu.FType = cutOperator) and (cpu.FOper = caeSub) and (cpu.FNext <> nil) and
      ((cpu.FNext.FType = cutConstInt) or (cpu.FNext.FType = cutConstDW) or (cpu.FNext.FType = cutConstFloat) or
      (cpu.FNext.FType = cutOpen) or (cpu.FNext.FType = cutFun) or (cpu.FNext.FType = cutArray) or (cpu.FNext.FType = cutName)) then
      if (cpu.FPrev = nil) or ((cpu.FPrev.FType <> cutConstInt) and (cpu.FPrev.FType <> cutConstDW) and
        (cpu.FPrev.FType <> cutConstFloat) and (cpu.FPrev.FType <> cutClose) and (cpu.FPrev.FType <> cutCloseArray) and
        (cpu.FPrev.FType <> cutName)) then
        cpu.FType := cutOperatorUnary;
  ...
  while cpu <> nil do
  begin
    if cpu.FType = cutOperator then
    begin
      if (cpu.FPrev = nil) or (cpu.FNext = nil) or
        ((cpu.FPrev.FType <> cutName) and (cpu.FPrev.FType <> cutConstInt) and
        (cpu.FPrev.FType <> cutConstDW) and (cpu.FPrev.FType <> cutConstFloat) and (cpu.FPrev.FType <> cutConstStr) and
        (cpu.FPrev.FType <> cutClose) and (cpu.FPrev.FType <> cutCloseArray)) or
        ((cpu.FNext.FType <> cutName) and (cpu.FNext.FType <> cutConstInt) and
        (cpu.FNext.FType <> cutConstDW) and (cpu.FNext.FType <> cutConstFloat) and (cpu.FNext.FType <> cutConstStr) and
        (cpu.FNext.FType <> cutOpen) and (cpu.FNext.FType <> cutFun) and (cpu.FNext.FType <> cutArray) and
        (cpu.FNext.FType <> cutOperatorUnary)) then
      begin
        result := CErrorEXP(0, cpu.FSme);
        cp.Free;
        exit;
    ...
    else if cpu.FType = cutAssume then
    begin
      if (cpu.FPrev = nil) or (cpu.FNext = nil) or
    ...
    else if cpu.FType = cutOperatorUnary then
      if (cpu.FNext = nil) or ((cpu.FNext.FType <> cutName) and (cpu.FNext.FType <> cutConstInt) and
        (cpu.FNext.FType <> cutConstDW) and (cpu.FNext.FType <> cutConstFloat) and (cpu.FNext.FType <> cutConstStr) and
        (cpu.FNext.FType <> cutOpen) and (cpu.FNext.FType <> cutFun) and (cpu.FNext.FType <> cutArray) and
        (cpu.FNext.FType <> cutOperatorUnary)) then
      begin
  ...
  begin
    ...
    else if cpu.FType = cutConstDW then
    begin
      cpu.FType := cutVar;
      cpu.FVar := VarAdd;
      exvar := EVar[cpu.FVar];
    ...
    else if cpu.FType = cutConstFloat then
    begin
      cpu.FType := cutVar;
      cpu.FVar := VarAdd;
      exvar := EVar[cpu.FVar];
    ...
    else if cpu.FType = cutConstStr then
    begin
      cpu.FType := cutVar;
      cpu.FVar := VarAdd;
      exvar := EVar[cpu.FVar];
    ...
    else if cpu.FType = cutName then
    begin
      cpu.FType := cutVar;
      cpu.FVar := VarAdd;
      exvar := EVar[cpu.FVar];
    ...
    else if cpu.FType = cutArray then
    begin
      cpu.FVar := VarAdd;
      exvar := EVar[cpu.FVar];
      exvar.FType := evteExtern;
  ...
  begin
    ...
    else if (cpu.FType = cutFun) or (cpu.FType = cutArray) then
    begin
      cnt := 0;
      cpu3 := cpu.FNext;
      while cpu3 <> nil do
      begin
        if cpu3.FType = cutVar then
          Inc(cnt)
        else if (cpu3.FType = cutClose) or (cpu3.FType = cutCloseArray) then
          break;
        cpu3 := cpu3.FNext;
      end;
      if (cpu.FType = cutArray) and (cnt < 1) then
      begin
        Clear;
        result := CErrorEXP(0, cpu.FSme);
        cp.Free;
      ...
      no := VarAdd;
      if cpu.FType = cutFun then
        EVar[no].FType := evteTemp
      else
        EVar[no].FType := evteArrayResult;
      elinstr := Instr[InstrAdd()];
      if cpu.FType = cutFun then
        elinstr.FType := eieFun
      else
        elinstr.FType := eieArray;
      ...
      while cpu2 <> nil do
      begin
        if cpu2.FType = cutVar then
        begin
          elinstr.FIndex[i] := cpu2.FVar;
          Inc(i);
        end
        else if (cpu2.FType = cutClose) or (cpu2.FType = cutCloseArray) then
          break;
        cpu2 := cpu2.FNext;
      end;
      cpu.FType := cutVar;
    ...
    begin
      ...
      EVar[no].FType := evteTemp;
      elinstr := Instr[InstrAdd()];
      if cpu.FOper = caeSub then
        elinstr.FType := eieMinus
      else
...
//                    raise ExceptionExpressionEC.Create('Unknown error');
    ...
    while (cpu2 <> nil) and (cpu2.FPrev <> nil) and (cpu2.FPrev.FType = cutOpen) and
      (cpu2.FNext <> nil) and (cpu2.FNext.FType = cutClose) do
    begin
...
begin
  ...
  begin
    exvar := EVar[i];
    if (exvar.FVar = nil) and (exvar.FType = evteExtern) then
...
begin
  ...
  begin
    exvar := EVar[i];
    if (exvar.FType = evteExtern) then
    begin
...
begin
  ...
  begin
    uva := va.Items[i];
    for u := 0 to FVarCount - 1 do
    begin
      exvar := EVar[u];
      if (exvar.FType = evteExtern) and (exvar.FName = uva.FName) then
        exvar.FVar := nil;
    end;
  end;
end;
...
begin
  ...
  begin
    exvar0 := EVar[i];
    if exvar0.FType = evteArrayResult then
      exvar0.FVar := nil;
    Inc(i);
  end;

  i := 0;
  while i < FInstrCount do
  begin
    eli := Instr[i];
    if (eli.FType = eieMinus) or (eli.FType = eieBitNot) or (eli.FType = eieNot) then
    begin
{            exvar0:=EVar[eli.FIndex[0]];
    ...
    else if (eli.FType <> eieFun) and (eli.FType <> eieAssume) and (eli.FType <> eieArray) then
    begin
{            exvar0:=EVar[eli.FIndex[0]];
            exvar1:=EVar[eli.FIndex[1]];
      ...
      vtt := vtUnknown;
      if (exvar0.FVar = nil) and (exvar0.FType = evteTemp) then
        if (eli.FType = eieAdd) or (eli.FType = eieSub) or (eli.FType = eieMul) or (eli.FType = eieDiv) then
          vtt := exvar1.GetVar.VType
        else
          vtt := vtInt;
    ...
    else if eli.FType = eieAssume then
    begin
{            exvar0:=EVar[eli.FIndex[0]];
            exvar1:=EVar[eli.FIndex[1]];
    ...
    else if eli.FType = eieArray then
    begin
{            exvar0:=EVar[eli.FIndex[0]];
            exvar1:=EVar[eli.FIndex[1]];
            tvar:=exvar1.FVar;
      ...
      exvar1 := EVar[eli.FIndex[1]];
      tvar := exvar1.GetVar;
      if (tvar.VType <> vtArray) then
        raise ExceptionExpressionEC.Create('Not array:' + exvar1.FName);

      for u := 2 to eli.FCount - 1 do
      begin
        exvar2 := EVar[eli.FIndex[u]];
        tvar2 := exvar2.GetVar;
        //                if (exvar2.FVar=nil) then raise ExceptionExpressionEC.Create('Not link array['+IntToStr(u-2)+'] :'+exvar2.FName);
        if tvar2.VType = vtStr then
        begin
          tvar := tvar.VArray.GetVarNE(tvar2.VStr);
          if tvar = nil then
            raise ExceptionExpressionEC.Create('Error array. index=' + exvar2.FVar.VStr);
        end
        else
        begin
          tvar := tvar.VArray.ItemsTB[tvar2.VInt];
          if tvar = nil then
            raise ExceptionExpressionEC.Create('Error array. index=' + IntToStr(exvar2.FVar.VInt));
        end;
        if (u <> eli.FCount - 1) and (tvar.VType <> vtArray) then
          raise ExceptionExpressionEC.Create('Error array:' + exvar1.FName);
      end;
      exvar0.FVar := tvar;
    end
    else
...
{            exvar0:=EVar[eli.FIndex[0]];
      ...
      if (exvar1var.VType = vtLibraryFun) then
      begin

        tvar := exvar1var.GetByRef();
        if (eli.FCount - 2) <> (High(tvar.FVLibraryFun) + 1 - 2) then
          raise ExceptionExpressionEC.Create('Count variable : ' + exvar1.FName);

        for u := (eli.FCount - 2 - 1) downto 0 do
        begin
        ...
        if tvar.FVLibraryFun[0] = 1 then
          exvar0.FVar.VInt := integer(dwzn)
        else if tvar.FVLibraryFun[0] = 2 then
...
{                dw:=CreateFile('1.txt',
      ...
      else if (exvar1var.VType = vtExternFun) then
      begin
      ...
      else if (exvar1var.VType = vtFun) then
      begin
        //            funBaseVarCount
        if exvar1var.VFun.FLocalVar.GetVar('funBaseVarCount').VInt < (eli.FCount - 2) then
          raise ExceptionExpressionEC.Create('Count var error. fun:' + exvar1.FName);
        ...
        begin
          exvar2var := EVar[eli.FIndex[u]].GetVar;
          //                    if exvar2.FVar=nil then raise ExceptionExpressionEC.Create('Not link var '+IntToStr(u-2)+': '+exvar2.FName);
          if tcfun.FLocalVar.Items[u - 2].RealVType = vtRef then
        ...
        raise ExceptionExpressionEC.Create('Not fun:' + exvar1.FName);
    end;
...
begin
  if (FRet < 0) or (EVar[FRet].FVar = nil) then
...
begin
  ...
  begin
    ev := EVar[i];
    if ev.FType = evteExtern then
      result := result + level + IntToStr(i) + '-Extern ' + ev.FName + #13#10
    else if ev.FType = evteTemp then
    begin
      result := result + level + IntToStr(i) + '-Temp';
      if ev.FVar <> nil then
        result := result + ' ' + ev.FVar.VStr;
      result := result + #13#10;
    end
    else if ev.FType = evteArrayResult then
      result := result + level + IntToStr(i) + '-AResult' + #13#10
    else
      raise Exception.Create('TExpressionEC.EndCode');
  end;

  for i := 0 to FInstrCount - 1 do
  begin
    ei := Instr[i];
    tstr := '';
    case ei.FType of
...
begin
  ...
  while el <> nil do
  begin
    if el.FType = cutOperatorUnary then
    begin
      if (el.FNext <> nil) and (el.FNext.FType = cutVar) then
        if l[0] = nil then
          l[0] := el;
    end
    else if el.FType = cutOperator then
    begin
      if (el.FPrev.FType = cutVar) and (el.FNext.FType = cutVar) then
      begin
        el2 := el.FNext.FNext;
        while el2 <> nil do
        begin
          if (el2.FType = cutOpen) or (el2.FType = cutOpenArray) or (el2.FType = cutFun) then
            break;
          if (el2.FType = cutClose) or (el2.FType = cutCloseArray) then
            break;
          el2 := el2.FNext;
        end;
        if (el2 = nil) or (not ((el2.FType = cutOpen) or (el2.FType = cutOpenArray) or (el2.FType = cutFun))) then
          if (el.FOper = caeMul) or (el.FOper = caeDiv) or (el.FOper = caeMod) then
          begin
            if l[1] = nil then
              l[1] := el;
          end
          else if (el.FOper = caeAdd) or (el.FOper = caeSub) then
          begin
            if l[2] = nil then
              l[2] := el;
          end
          else if (el.FOper = caeShl) or (el.FOper = caeShr) then
          begin
            if l[3] = nil then
              l[3] := el;
          end
          else if (el.FOper = caeEqual) or (el.FOper = caeNotEqual) or (el.FOper = caeLess) or (el.FOper = caeMore) or
            (el.FOper = caeLessEqual) or (el.FOper = caeMoreEqual) then
          begin
            if l[4] = nil then
              l[4] := el;
          end
          else if (el.FOper = caeBitAnd) then
          begin
            if l[5] = nil then
              l[5] := el;
          end
          else if (el.FOper = caeBitXor) then
          begin
            if l[6] = nil then
              l[6] := el;
          end
          else if (el.FOper = caeBitOr) then
          begin
            if l[7] = nil then
              l[7] := el;
          end
          else if (el.FOper = caeAnd) then
          begin
            if l[8] = nil then
              l[8] := el;
          end
          else if (el.FOper = caeOr) then
            if l[9] = nil then
              l[9] := el;
      end;
    end
    else if el.FType = cutAssume then
      if (el.FPrev.FType = cutVar) and (el.FNext.FType = cutVar) then
        if l[10] = nil then
          l[10] := el;
    el := el.FNext;
...
begin
  ...
  while el <> nil do
  begin
    if el.FType = cutArray then
    begin
      el2 := el.FNext;
      while el2 <> nil do
      begin
        if (el2.FType = cutCloseArray) then
        begin
          result := el;
          exit;
        end;
        if (el2.FType <> cutComma) and (el2.FType <> cutVar) then
          break;
        el2 := el2.FNext;
      end;
    end;
...
begin
  ...
  while el <> nil do
  begin
    if el.FType = cutFun then
    begin
      el2 := el.FNext;
      while el2 <> nil do
      begin
        if (el2.FType = cutClose) then
        begin
          result := el;
          exit;
        end;
...
var
  ...
  i: integer;
begin
  for i := 0 to FTry.Count - 1 do
  begin
    ut := FTry.Items[i];
    HeapFree(GetProcessHeap(), 0, ut);
  end;
  for i := 0 to FThrow.Count - 1 do
  begin
    uth := FThrow.Items[i];
    if uth.FVar <> nil then
    begin
      uth.FVar.Free;
      uth.FVar := nil;
    end;
...
begin
  ...
  if tstr <> '' then
    raise ExceptionExpressionEC.Create(tstr);
end;
...
begin
  ...
  while cu <> nil do
  begin
    if cu.FJump <> nil then
    begin
      cu2 := sou.FFirst;
      cudes2 := FFirst;
      while cu2 <> nil do
      begin
        if cu.FJump = cu2 then
          cudes.FJump := cudes2;
        cu2 := cu2.FNext;
        cudes2 := cudes2.FNext;
      end;
...
begin
  ...
  if result = nil then
    ...
    begin
      ...
      if (tv.FVType = vtFun) and (tv.FVFun.FIsClass) then
      begin
...
function TCodeEC.Compiler(ca: TCodeAnalyzerEC; codeunit: integer; funinclude: FunctionIncludeUnitEC;
  el: TCodeAnalyzerUnitEC; elendcomp: PDWORD; curclass: TCodeEC): WideString;
begin
  //    Clear;
  result := '';
...
{$WARNINGS OFF}
function TCodeEC.CompilerR(ca: TCodeAnalyzerEC; codeunit: integer; funinclude: FunctionIncludeUnitEC;
  el: TCodeAnalyzerUnitEC; insertto: TCodeUnitEC; elendcomp: PDWORD; elcontinue: PDWORD;
  opBreak: TCodeUnitEC; opContinue: TCodeUnitEC; curclass: TCodeEC): WideString;
var
  ...
  begin
    if tstr = 'unknown' then
      FLocalVar.Add(Name, vtUnknown)
    else if tstr = 'int' then
      FLocalVar.Add(Name, vtInt)
    else if tstr = 'dword' then
      FLocalVar.Add(Name, vtDW)
    else if tstr = 'float' then
      FLocalVar.Add(Name, vtFloat)
    else if tstr = 'str' then
      FLocalVar.Add(Name, vtStr)
    else if tstr = 'ref' then
      FLocalVar.Add(Name, vtRef)
    else if tstr = 'array' then
      FLocalVar.Add(Name, vtArray);
  end;
  ...
  begin
    result := (el <> nil) and (el.FNext <> nil) and (el.FNext.FType = caeName) and (el.FType = caeName) and
      ((el.FStr = 'unknown') or (el.FStr = 'int') or (el.FStr = 'dword') or (el.FStr = 'float') or
      (el.FStr = 'str') or (el.FStr = 'ref') or (el.FStr = 'array')) and (IsVarEXP(el.FNext.FStr));
  end;
  ...
  begin
    ...
    oper := el.FStr;
    el := el.FNext;
    while (el.FType = caeName) and IsVarEXP(el.FStr) do
    begin
      if FLocalVar.GetVarNE(el.FStr) <> nil then
      begin
        result := CErrorEXP(0, el.FSme);
        exit;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if InDeclareVar(el) then
      begin
        result := DeclareVar(el);
        if result <> '' then
          exit;

        if el.FType <> caeSemicolon then
        begin
          result := CErrorEXP(0, el.FSme);
          exit;
        end;
        ...
        if (elcontinue <> nil) and (countopen = 0) then
        begin
          elcontinue^ := DWORD(el);
          exit;
        end;
      ...
      else if oper = 'if' then
      begin
        unempty := UnitInsert(insertto);
        unempty.FType := cuteEmpty;
        unempty.FSme := unempty.FSme;
        unempty.FLen := 0;
        unempty.FUnit := codeunit;
        while true do
        begin
          if el.FNext = nil then
          begin
          ...
          if (el.FNext.FType <> caeOpen1) or (el.FNext.FNext = nil) then
          begin
            result := CErrorEXP(0, el.FNext.FSme);
            exit;
          end;

          un := UnitInsert(unempty);
          un.FType := cuteIf;
          un.FSme := el.FSme;
          un.FLen := 0;
          un.FUnit := codeunit;
          un.FJump := unempty;
          un.FExperssion := TExpressionEC.Create;
          result := un.FExperssion.Compiler(ca, curclass, el.FNext.FNext, nil, @el2);
          if result <> '' then
            exit;
          if el2 = nil then
          begin
          ...
          if el2.FType <> caeClose1 then
          begin
            result := CErrorEXP(0, el2.FSme);
            exit;
          end;
          el := el2.FNext;
          ...
          begin
            result := CompilerR(ca, codeunit, funinclude, el, un2, elendcomp, @el, opBreak, opContinue, curclass);
            if result <> '' then
              exit;
          end;
          ...
          begin
            ...
            begin
              ...
              if (el.FNext <> nil) and (el.FNext.FType = caeName) and (LowerCase(el.FNext.FStr) = 'if') then
              begin
                el := el.FNext;
                continue;
              end;
              ...
              if el.FNext.FType <> caeSemicolon then
              begin
                result :=
                  CompilerR(ca, codeunit, funinclude, el.FNext, unempty, elendcomp, @el, opBreak, opContinue, curclass);
                if result <> '' then
                  exit;
                if (elcontinue <> nil) and (countopen = 0) then
                begin
      ...
      begin
        ...
        begin
          result := CompilerR(ca, codeunit, funinclude, el, un, elendcomp, @el, unempty, un2, curclass);
          if result <> '' then
            exit;
        end;
      ...
      begin
        ...
        if (el.FNext.FType <> caeOpen1) then
        begin
          result := CErrorEXP(0, el.FNext.FSme);
          exit;
        end;
        if (el.FNext.FNext = nil) then
        begin
          result := CErrorEXP(0, el.FNext.FSme + el.FNext.FLen);
          exit;
        end;

        el := el.FNext.FNext;
        if el.FType = caeSemicolon then
        else if InDeclareVar(el) then
        begin
          result := DeclareVar(el);
          if result <> '' then
            exit;
          ...
          begin
            ...
            if el2.FType = caeSemicolon then
            begin
              el := el2;
              break;
            end
            else if el2.FType = caeComma then
              el := el2.FNext
            else
            begin
              result := CErrorEXP(0, el2.FSme);
        ...
        begin
          ...
          if el2.FNext = nil then
          begin
            result := CErrorEXP(0, el2.FSme + el2.FLen);
            exit;
          end;
          el := el2.FNext;
      ...
      else if oper = 'break' then
      begin
        if (opBreak = nil) or (el.FNext = nil) then
        begin
          result := CErrorEXP(0, el.FSme);
          exit;
        end;
        ...
        un.FType := cuteJump;
        un.FSme := el.FSme;
        un.FLen := el.FNext.FSme - el.FSme + el.FNext.FLen;
        un.FUnit := codeunit;
        un.FJump := opBreak;
      ...
      else if oper = 'continue' then
      begin
        if (opContinue = nil) or (el.FNext = nil) then
        begin
          result := CErrorEXP(0, el.FSme);
          exit;
        end;
      ...
      else if (oper = '#include') or (oper = '#insert') then
      begin
        if el.FNext = nil then
        begin
          result := CErrorEXP(0, el.FSme + el.FLen);
        ...
        if (el.FNext.FType <> caeStr) then
        begin
          result := CErrorEXP(0, el.FNext.FSme);
          exit;
        end;

        if not Assigned(funinclude) then
        begin
          result := CErrorEXP(0, el.FSme);
          exit;
        end;
        ...
        begin
          ...
          result := CompilerR(cainclude, unitinclude, funinclude, cainclude.FFirst,
            insertto, nil, nil, nil, nil, curclass);
          if result <> '' then
          begin
            cainclude.Free;
            exit;
      ...
      begin
        ...
        if (el.FNext.FType <> caeName) or (not IsVarEXP(el.FNext.FStr)) or (el.FNext.FNext = nil) then
        begin
          result := CErrorEXP(0, el.FNext.FSme);
          exit;
        end;
        if (el.FNext.FNext.FType <> caeOpen1) or (el.FNext.FNext.FNext = nil) then
        begin
          result := CErrorEXP(0, el.FNext.FNext.FSme);
          exit;
        end;
        ...
        while (el2 <> nil) and (el2.FType = caeName) and (IsVarEXP(el2.FStr)) do
        begin
          oper := LowerCase(el2.FStr);
          if (oper = 'unknown') or (oper = 'int') or (oper = 'dword') or (oper = 'float') or
            (oper = 'str') or (oper = 'ref') or (oper = 'array') then
            el2 := el2.FNext
          else
            oper := 'unknown';
          if (el2 = nil) or (el2.FType <> caeName) or (not IsVarEXP(el2.FStr)) then
            break;

          tvar := nil;
          if oper = 'unknown' then
            tvar := cfun.FLocalVar.Add(el2.FStr, vtUnknown)
          else if oper = 'int' then
            tvar := cfun.FLocalVar.Add(el2.FStr, vtInt)
          else if oper = 'dword' then
            tvar := cfun.FLocalVar.Add(el2.FStr, vtDW)
          else if oper = 'float' then
            tvar := cfun.FLocalVar.Add(el2.FStr, vtFloat)
          else if oper = 'str' then
            tvar := cfun.FLocalVar.Add(el2.FStr, vtStr)
          else if oper = 'ref' then
            tvar := cfun.FLocalVar.Add(el2.FStr, vtRef)
          else if oper = 'array' then
            tvar := cfun.FLocalVar.Add(el2.FStr, vtArray);

          el2 := el2.FNext;
          if el2 = nil then
          begin
            result := CErrorEXP(0, ca.FLast.FSme + ca.FLast.FLen);
            exit;
          end;

          if (oper <> 'ref') and (el2.FType = caeAssume) then
          begin
            if el2.FNext = nil then
            begin
              result := CErrorEXP(0, ca.FLast.FSme + ca.FLast.FLen);
            ...
            if DecodeFloatEXP(el2, zfloat) then
              tvar.VFloat := zfloat
            else if DecodeDWEXP(el2, zdword) then
              tvar.VDW := zdword
            else if DecodeIntEXP(el2, zint) then
              tvar.VInt := zint
            else if DecodeStrEXP(el2, zstr) then
          ...
          if (el2 = nil) or (el2.FType <> caeComma) then
            break;
          el2 := el2.FNext;
        end;
        if (el2 = nil) or (el2.FType <> caeClose1) then
        begin
          result := CErrorEXP(0, el.FNext.FNext.FNext.FSme);
          exit;
        end;
        el := el2.FNext;
        if el = nil then
        ...
        if (el.FType <> caeOpen2) or (el.FNext = nil) then
        begin
          result := CErrorEXP(0, el.FSme);
          exit;
        end;
        ...
        el2 := nil;
        result := cfun.Compiler(ca, codeunit, funinclude, el.FNext, @el2, curclass);
        if result <> '' then
          exit;
        if el2 = nil then
        begin
      ...
      begin
        ...
        if (el.FNext.FType <> caeName) or (not IsVarEXP(el.FNext.FStr)) then
        begin
          result := CErrorEXP(0, el.FNext.FSme);
          exit;
        end;
        ...
        begin
          ...
          begin
            ...
            if (tvar2 = nil) or (tvar2.VType <> vtFun) then
            begin
              result := CErrorEXP(0, el.FSme);
              exit;
            end;
        ...
        el2 := nil;
        result := cfun.Compiler(ca, codeunit, funinclude, el.FNext, @el2, cfun);
        if result <> '' then
          exit;
        if el2 = nil then
        begin
      ...
      begin
        ...
        if (el.FNext.FType <> caeOpen2) then
        begin
          result := CErrorEXP(0, el.FNext.FSme);
          exit;
        end;

        un2 := UnitInsert(insertto);
        un2.FType := cuteTry;
        un2.FSme := el.FSme;
        un2.FLen := 0{el.FLen};
        un2.FUnit := codeunit;

        result := CompilerR(ca, codeunit, funinclude, el.FNext, insertto, elendcomp, @el, nil, nil, curclass);
        if result <> '' then
          exit;
        ...
        if el.FType <> caeName then
        begin
          result := CErrorEXP(1001, el.FSme);
          exit;
        end;
        if (el.FStr <> 'catch') and (el.FStr <> 'finally') then
        begin
          result := CErrorEXP(1001, el.FSme);
          exit;
        end;
        if (el.FNext = nil) then
        begin
          result := CErrorEXP(1001, el.FSme + el.FLen);
          exit;
        end;

        if el.FStr = 'catch' then
          zint := 0
        else
          zint := 1;
        el := el.FNext;
        ...
        result := CompilerR(ca, codeunit, funinclude, el, un{unempty}, elendcomp, @el, nil, nil, curclass);
        if result <> '' then
          exit;
        if (elcontinue <> nil) and (countopen = 0) then
        begin
      ...
      begin
        ...
        begin
          ...
          un.FUnit := codeunit;
          un.FExperssion := TExpressionEC.Create;
          result := un.FExperssion.Compiler(ca, curclass, el.FNext, nil, @el2);
          if result <> '' then
            exit;
          if el2 = nil then
          begin
    ...
    else if el.FType = caeOpen2 then
      Inc(countopen)
    else if el.FType = caeClose2 then
    begin
      Dec(countopen);
      if (elcontinue <> nil) and (countopen = 0) then
      begin
      ...
      else if countopen = -1 then
      begin
        if elendcomp <> nil then
          elendcomp^ := DWORD(el);
        exit;
      end;
    end
...
begin
  ...
  begin
    ...
    if un.FExperssion <> nil then
      un.FExperssion.Link(va);
    un := un.FNext;
  end;

  for i := 0 to FLocalVar.Count - 1 do
    with FLocalVar.Items[i] do
      if (FVType = vtFun) and (VFun <> nil) then
        VFun.Link(va)
      else if (FVType = vtClass) and (VFun <> nil) then
        VClass.Link(va);
end;
...
begin
  ...
  for i := 0 to FLocalVar.Count - 1 do
    with FLocalVar.Items[i] do
      if (FVType = vtFun) and (VFun <> nil) then
        VFun.LinkAll(va)
      else if (FVType = vtClass) and (VClass <> nil) then
        VClass.LinkAll(va);
end;
...
begin
  for i := 0 to FLocalVar.Count - 1 do
    with FLocalVar.Items[i] do
      if (FVType = vtFun) and (FVFun <> nil) and (FVFun.FIsClass) then
        FVFun.LinkClass;
  LinkAll(FLocalVar);
end;
...
begin
  ...
  begin
    if un.FType = cuteExpression then
      un.FExperssion.UnLink(va);
    un := un.FNext;
  end;

  for i := 0 to FLocalVar.Count - 1 do
    with FLocalVar.Items[i] do
      if (VType = vtFun) and (VFun <> nil) then
        VFun.UnLink(va)
      else if (VType = vtClass) and (VClass <> nil) then
        VClass.UnLink(va);
end;
...
begin
  ...
  begin
    ...
    else if un.FType = cuteJump then
    begin
      un := un.FJump;
      continue;
    end
    else if un.FType = cuteIf then
    begin
      un.FExperssion.Run(cpr, self);
      if not un.FExperssion.Return.IsTrue then
      begin
        un := un.FJump;
        continue;
      end;
    end
    else if un.FType = cuteExit then
    begin
      while true do
      begin
        pt := cpr.TryGetLast;
        if (pt = nil) or (pt.FCode <> self) then
          break;
        cpr.TryDelLast;
      end;
      break;
    end
    else if un.FType = cuteTry then
      cpr.TryAdd(self, un.FJump)
    else if un.FType = cuteTryEnd then
      cpr.TryDelLast
    else if un.FType = cuteThrow then
      if un.FExperssion <> nil then
      begin
        un.FExperssion.Run(cpr, self);
        cpr.ThrowAdd(un.FExperssion.Return);
      end
      else if throwvar <> nil then
      begin
...
begin
  ...
  begin
    ...
    if rz = WAIT_FAILED then
      break;
    if (rz = WAIT_OBJECT_0) or (rz = WAIT_ABANDONED) then
      break;

    if (cd.FStop and (un.FLen > 0)) or un.FBP then
    begin
      cd.FStopUnit := un;
      ...
      if rz = WAIT_FAILED then
        break;
      if rz = WAIT_OBJECT_0 then
        break;
      if (rz >= WAIT_ABANDONED_0) and (rz < (WAIT_ABANDONED_0 + 2)) then
        break;

      cd.FStopLastCode := self;

      if cd.FStopType = cdst_StepIn then
        cd.FStop := true;
    end;
    if un.FType = cuteExpression then
...
{        end else if un.FType=cuteThrowEnd then begin
    ...
    if (cd.FStopType = cdst_StepNext) and (cd.FStopLastCode = self) then
      cd.FStop := true;
    un := un.FNext;
  end;

  if ((cd.FStopType = cdst_StepNext) or (cd.FStopType = cdst_StepOut)) and (cd.FStopLastCode = self) then
    cd.FStop := true;
...
begin
  ...
  begin
    ...
    begin
      case tv.FVType of
        ...
        vtArray: tstr := 'array';
        vtRef: tstr := 'ref';
        else raise Exception.Create('TCodeEC.EndCode');
      end;
  ...
  begin
    ...
    if un.FType = cuteEmpty then
      result := result + level + 'Empty'#13#10
    else if un.FType = cuteExpression then
    begin
...
begin
  ...
  if FFirst = nil then
  begin
    result := BPNearestFun(codeunit, tsme, tsmemin, tsmemax);
    exit;
  end;
  ...
  while un <> nil do
  begin
    if un.FUnit <> codeunit then
    begin
      un := un.FNext;
      continue;
    end;
    if (un.FLen > 0) and (un.FSme >= tsmemin) then
      break;
    un := un.FNext;
  end;
  if un = nil then
  ...
  if (un.FSme{+un.FLen}) >= tsmemax then
  begin
    result := BPNearestFun(codeunit, tsme, tsmemin, tsmemax);
    exit;
  end;
  ...
  result := un;
  if (tsme >= un.FSme) and (tsme < (un.FSme + un.FLen)) then
    exit;
  ...
  begin
    ...
    if (un.FLen > 0) then
      if mindist > abs(un.FSme - tsme) then
      begin
        mindist := abs(un.FSme - tsme);
        result := un;
      end// else Exit;
...
begin
  ...
  while un <> nil do
  begin
    if (un.FSme = el.FSme) and (un.FLen = el.FLen) and (un.FUnit = el.FUnit) and (un.FType = el.FType) then
      li.Add(un);
    un := un.FNext;
  end;
...
begin
  ...
  av[0].Assume(av[1]);
  for i := 2 to cnt - 1 do
    if av[i].Less(av[0]) then
    begin
      av[0].VType := av[i].VType;
      av[0].Assume(av[i]);
    end;
end;
...
begin
  ...
  for i := 0 to cnt - 1 do
  begin
    if (av[i + 1].VType <> vtInt) or (av[i + 1].VInt < 1) then
    begin
      ar := nil;
      exit;
    end;
...
begin
  ...
  if cnt < 1 then
    exit;
  if av[1].VType = vtArray then
    av[0].VInt := av[1].VArray.Count;
  if av[1].VType = vtStr then
    av[0].VInt := Length(av[1].VStr);
end;
...
begin
  ...
  if cnt < 2 then
    exit;
  if av[1].VType = vtInt then
    av[0].VInt := abs(av[1].VInt)
  else
    av[0].VFloat := abs(av[1].VFloat);
end;
...
begin
  ...
  if cnt < 2 then
    exit;
  if av[1].VType = vtFloat then
    av[0].VInt := Round(av[1].VFloat)
  else
    av[0].VInt := av[1].VInt;
end;
...
begin
  ...
  if cnt < 2 then
    exit;
  if av[1].VType = vtInt then
    av[0].VInt := av[1].VInt * av[1].VInt
  else
    av[0].VFloat := sqr(av[1].VFloat);
end;
...
begin
  ...
  if (sme < 0) or (sme >= lenstr) then
  begin
    av[0].VStr := '';
    exit;
  end;
  if sme + tlen > lenstr then
    tlen := lenstr - sme;
...
begin
  ...
  if av[2].VStr = 'int' then
    av[0].FVLibraryFun[0] := 1
  else if av[2].VStr = 'dword' then
    av[0].FVLibraryFun[0] := 2
  else
    av[0].FVLibraryFun[0] := 0;
  ...
  begin
    tstr := av[4 + i].VStr;
    if tstr = 'int' then
      av[0].FVLibraryFun[2 + i] := 1
    else if tstr = 'dword' then
      av[0].FVLibraryFun[2 + i] := 2
    else if tstr = 'str' then
      av[0].FVLibraryFun[2 + i] := 3
    else
...
begin
  ...
  if code = nil then
    exit;
  while (code <> nil) and (code.FParent <> nil){and (not code.FIsClass)} do
    code := code.FParent;
...
{    tstr:=code.FLocalVar.Items[0].FName;
  ...
  if tvar.VType <> vtFun then
    exit;
  tc := tvar.VFun;
  if not tc.FIsClass then
    exit;
