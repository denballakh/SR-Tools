...
begin
  ...
  while wu <> nil do
  begin
    if FPathIn.Find(wu.FFileName) < 0 then
    begin
...
procedure TFormUnitPath.DGLDrawCell(Sender: TObject; ACol, ARow: integer; Rect: TRect; State: TGridDrawState);
var
  tstr: WideString;
begin
  if (FCnt > 0) and (ARow >= 0) and (ARow < FCnt) then
  begin
...
procedure TFormUnitPath.DGLGetEditText(Sender: TObject; ACol, ARow: integer; var Value: string);
begin
  Value := FPathOut.Item[ARow];
end;

procedure TFormUnitPath.DGLSetEditText(Sender: TObject; ACol, ARow: integer; const Value: string);
begin
