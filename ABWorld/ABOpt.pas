...
begin
  ...
  for i := 0 to High(ab_OptGroup) do
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      while sp <> nil do
      begin
        if sp.FOwner.IndexOf(wu) >= 0 then
        begin
          sp2 := sp;
          if sp2.FParent <> nil then
            sp2 := sp2.FParent;
  ...
  begin
    orbangle := 0;
    while orbangle <= (180 + 0.001) do
    begin
      ab_Opt_Cur := @(ab_Opt[i]);

      if Zone_Exclude(orb, orbangle) then
        ab_Opt_Cur.FPoint := nil//              ab_Opt_Cur.FPolygon:=nil;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              while t < u do
              begin
                if ab_Opt_Cur.FPoint[t] = sp2 then
                  break;
                Inc(t);
              end;
              if t >= u then
...
begin
  ...
  begin
    ...
    for u := 0 to High(cg.FPolygon) do
      bd.AddWORD(cg.FPolygon[u].FNo);

    bd.AddSingle(cg.FBB0.x);
    bd.AddSingle(cg.FBB0.y);
    bd.AddSingle(cg.FBB0.z);
    bd.AddSingle(cg.FBB1.x);
    bd.AddSingle(cg.FBB1.y);
    bd.AddSingle(cg.FBB1.z);
    bd.AddSingle(cg.FBB2.x);
    bd.AddSingle(cg.FBB2.y);
    bd.AddSingle(cg.FBB2.z);
    bd.AddSingle(cg.FBB3.x);
    bd.AddSingle(cg.FBB3.y);
    bd.AddSingle(cg.FBB3.z);
  end;

  for i := 0 to ab_Opt_OrbCnt * ab_Opt_OrbAngleCnt - 1 do
  begin
    ab_Opt_Cur := @(ab_Opt[i]);
...
begin
  ...
  if no >= ab_Opt_OrbCnt then
    no := 0;

  noa := Round((ab_Camera_Pos.FOrbitAngle / 180) * (ab_Opt_OrbAngleCnt - 1));
  if noa >= ab_Opt_OrbAngleCnt then
    EError('ab_OptCur');// noa:=0;
