...
{  if FGraph=nil then begin
  ...
  if (FPointStart < 0) or (FPointEnd < 0) then
    exit;
...
procedure WorldLine_Clear;
begin
  while WorldLine_First <> nil do
    WorldLine_Delete(WorldLine_Last);

  if WorldLine_Grap <> nil then
  begin
    ab_Obj3D_Delete(WorldLine_Grap);
    WorldLine_Grap := nil;
  end;
end;
...
begin
  ...
  if WorldLine_Last <> nil then
    WorldLine_Last.FNext := el;
  el.FPrev := WorldLine_Last;
  el.FNext := nil;
  WorldLine_Last := el;
  if WorldLine_First = nil then
    WorldLine_First := el;
...
begin
  ...
  if WorldLine_Last = el then
    WorldLine_Last := el.FPrev;
  if WorldLine_First = el then
    WorldLine_First := el.FNext;
