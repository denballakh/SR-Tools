...
function OKGF_ReadStart_File(filename: PChar; lenx: PDWORD; leny: PDWORD): DWORD; cdecl;
  external 'okgf.dll' Name 'OKGF_ReadStart_File';
function OKGF_ReadStart_Buf(soubuf: Pointer; soubuflen: DWORD; lenx: PDWORD; leny: PDWORD): DWORD;
  cdecl; external 'okgf.dll' Name 'OKGF_ReadStart_Buf';
function OKGF_Read(id: DWORD; buf: Pointer; lenline: DWORD; mR, mG, mB, mD: DWORD; BytePP: DWORD): DWORD;
  cdecl; external 'okgf.dll' Name 'OKGF_Read';
...
begin
  ...
  if id = 0 then
    raise Exception.Create('TabTex.LoadFile. Error load file:' + path);

  if GR_lpDev.CreateTexture(lenx, leny, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, FTex) <> D3D_OK then
    EError('CreateTexture');

  if FTex.LockRect(0, @d3dlr, nil, 0) <> D3D_OK then
    EError('Tex.Lock');
  ...
  if id = 0 then
    raise Exception.Create('TGraphBuf.LoadRGBA. Error load file:' + path);
...
procedure ab_Tex_Clear();
begin
  while ab_Tex_First <> nil do
    ab_Tex_Delete(ab_Tex_Last);
end;
...
begin
  ...
  if ab_Tex_Last <> nil then
    ab_Tex_Last.FNext := el;
  el.FPrev := ab_Tex_Last;
  el.FNext := nil;
  ab_Tex_Last := el;
  if ab_Tex_First = nil then
    ab_Tex_First := el;
...
begin
  ...
  if ab_Tex_Last = el then
    ab_Tex_Last := el.FPrev;
  if ab_Tex_First = el then
    ab_Tex_First := el.FNext;
