...
begin
  ...
  begin
    ...
    if not FormMain.MM_View_Point.Checked then
...
begin
  ...
  if bp.Par_Count('PortId') > 0 then
    FPortId := bp.Par['PortId']
  else
    FPortId := '';
  if bp.Par_Count('PortType') > 0 then
    FPortType := bp.Par['PortType']
  else
    FPortType := '';
  if bp.Par_Count('PortLink') > 0 then
    FPortLink := bp.Par['PortLink']
  else
    FPortLink := '';
end;
...
procedure Point_Clear;
begin
  while Point_First <> nil do
    Point_Delete(Point_Last);
end;
...
begin
  ...
  if Point_Last <> nil then
    Point_Last.FNext := el;
  el.FPrev := Point_Last;
  el.FNext := nil;
  Point_Last := el;
  if Point_First = nil then
    Point_First := el;
...
begin
  ...
  if Point_Last = el then
    Point_Last := el.FPrev;
  if Point_First = el then
    Point_First := el.FNext;
...
begin
  ...
  while el <> nil do
  begin
    if el.FOwner.IndexOf(own) >= 0 then
      li.Add(el);
    el := el.FNext;
  end;
...
begin
  ...
  while ali <> nil do
  begin
    if ali.FVerStart = sou then
      ali.FVerStart := des;
    if ali.FVerEnd = sou then
      ali.FVerEnd := des;
    ali := ali.FNext;
  end;
...
begin
  if des = sou then
    exit;
  if sou.FParent = des then
    exit;
  if (des.FParent <> nil) and (sou.FParent = des.FParent) then
    exit;

  oldpar := sou.FParent;

  if des.FParent = nil then
    sou.FParent := des
  else
...
begin
  ...
  begin
    if el.FParent <> nil then
      el.FParent := Point_List[integer(el.FParent)];//Point_ByNo(integer(el.FParent));
    el := el.FNext;
  end;
end;
