...
type
  TFormMain = class(TForm)
    ...
    procedure EditPointPosKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
    procedure ButtonTriInvertClick(Sender: TObject);
    procedure ButtonTriDeleteClick(Sender: TObject);
    procedure EditTPColorKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
    ...
    procedure EditTPTexKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
    ...
    procedure EditLPColorKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
...
begin
  ...
  for i := GSortTriangle.Count - 1 downto 0 do
...
begin
  ...
  if (cntsel <> 1) and (NInfo.ActivePage = 'Point') then
    newp := 'Main'
  else if cntsel = 1 then
    newp := 'Point'
  else
    newp := 'Main';
  ...
  begin
    atr := Triangle_Sel;
    if atr <> nil then
      if atr.FSelPoint < 0 then
        newp := 'Triangle'
      else
        newp := 'TriPoint';
  end;

  if NInfo.ActivePage <> newp then
    NInfo.ActivePage := newp;

  if NInfo.ActivePage = 'Point' then
  begin
    apo := Point_Sel;
  ...
  else if NInfo.ActivePage = 'Triangle' then
  begin
    ...
    while atr2 <> nil do
    begin
      if atr2.FTexture <> '' then
      begin
        tstr := File_Name(atr2.FTexture);
        if EditTriTexture.Items.IndexOf(tstr) < 0 then
        begin
  ...
  else if NInfo.ActivePage = 'TriPoint' then
  begin
    i := atr.FSelPoint;
    ...
    EditTPColor.Text := Format('%d,%d,%d,%d', [D3DCOLOR_A(atr.FV[i].FColor.VDWORD), D3DCOLOR_R(
      atr.FV[i].FColor.VDWORD), D3DCOLOR_G(atr.FV[i].FColor.VDWORD), D3DCOLOR_B(atr.FV[i].FColor.VDWORD)]);
  ...
  else if NInfo.ActivePage = 'Line' then
    EditLineType.Text := ali.FType
  else if NInfo.ActivePage = 'LinePoint' then
  begin
    if ali.FSelPoint = 0 then
      dw := ali.FColorStart.VDWORD
    else
      dw := ali.FColorEnd.VDWORD;
...
begin
  ...
  if ali <> nil then
    if ali.FSelPoint = 0 then
      key := ali.FColorStart
    else if ali.FSelPoint = 1 then
      key := ali.FColorEnd;

  atr := Triangle_Sel;
  if atr <> nil then
    if atr.FSelPoint >= 0 then
      key := atr.FV[atr.FSelPoint].FColor;
...
begin
  ...
  Handled := false;
  if Msg.message = ($400 + 123) then
  begin
    if CheckBoxPlay.Checked then
      SetTime(Key_Time + 20);
  end
  else if (Msg.message = WM_PAINT) and (Msg.hwnd = Panel3D.Handle) then
  begin
    Draw3D;
    Handled := true;
  end
  else if (Msg.message = WM_KEYDOWN) and (not ed) then
    if _ctrl and (Msg.wParam = integer('A')) then
    begin
      ClacTexCoord;
    ...
    begin
      ...
      GetCursorPos(tp);
      if CalcWorld(Panel3D.ScreenToClient(tp), v) then
      begin
        apo := Point_Add;
        apo.Pos := v;
        apo.Sel := true;
    ...
    else if _ctrl and (Msg.wParam = integer('T')) then
    begin
      while true do
      begin
        apo1 := Point_Sel;
        ...
        if apo3 = nil then
          break;
        if Point_Sel(apo3) <> nil then
          break;
        if Triangle_Find(apo1, apo2, apo3) <> nil then
          break;
    ...
    else if _ctrl and (Msg.wParam = integer('L')) then
    begin
      while true do
      begin
        apo1 := Point_Sel;
        ...
        if apo2 = nil then
          break;
        if Point_Sel(apo2) <> nil then
          break;
        if Line_Find(apo1, apo2) <> nil then
          break;
    ...
    else if (Msg.wParam = VK_LEFT) or (Msg.wParam = VK_RIGHT) then
    begin
    ...
    else if (Msg.wParam = VK_UP) or (Msg.wParam = VK_DOWN) then
    begin
    ...
    else if _ctrl and (Msg.wParam = integer('C')) then
    begin
    ...
    else if (Msg.wParam = VK_PRIOR) or (Msg.wParam = VK_NEXT) then
    begin
    ...
    else if (Msg.wParam = VK_HOME) then
    begin
      Camera_Pos := D3DV(0, 0, -3500);
      Camera_Des := D3DV(0, 0, 0);
      Camera_Up := D3DV(0, 1, 0);
...
begin
  if (FMove) or (FRotateCamera) then
  begin
    FMove := false;
    FRotateCamera := false;
  ...
  begin
    if D3D_IntersectTriangle(vpos, vdir, atr.FV[1].FVer.FPos, atr.FV[0].FVer.FPos, atr.FV[2].FVer.FPos, t, u, v) or
      ...
      begin
        ...
        while i < li.Count do
        begin
          if TTriangleAB(li.Items[i]).FPickT > t then
            break;
          Inc(i);
        end;
        li.Insert(i, atr);
...
begin
  ...
  if ali <> nil then
    if ali.FSelPoint = 0 then
      result := ali.FColorStart
    else if ali.FSelPoint = 1 then
      result := ali.FColorEnd;
  atr := Triangle_Sel;
  if atr <> nil then
    if atr.FSelPoint >= 0 then
      result := atr.FV[atr.FSelPoint].FColor;
end;
...
begin
  ...
  if (Button = mbLeft) and (not FMove) and (not FRotateCamera) then
  begin
    atr := Triangle_Sel;
    ali := Line_Sel;

    if (atr <> nil) and (ssShift in Shift) then
    begin
      zn := sqr(atr.FV[0].FVer.FPosShow.x - v.x) + sqr(atr.FV[0].FVer.FPosShow.y - v.y);
      i := 0;
      zn2 := sqr(atr.FV[1].FVer.FPosShow.x - v.x) + sqr(atr.FV[1].FVer.FPosShow.y - v.y);
      if zn2 < zn then
      begin
        zn := zn2;
        i := 1;
      end;
      zn2 := sqr(atr.FV[2].FVer.FPosShow.x - v.x) + sqr(atr.FV[2].FVer.FPosShow.y - v.y);
      if zn2 < zn then
        i := 2;
    ...
    else if (ali <> nil) and (ssShift in Shift) then
    begin
      zn := sqr(ali.FVerStart.FPosShow.x - v.x) + sqr(ali.FVerStart.FPosShow.y - v.y);
      i := 0;
      zn2 := sqr(ali.FVerEnd.FPosShow.x - v.x) + sqr(ali.FVerEnd.FPosShow.y - v.y);
      if zn2 < zn then
        i := 1{zn:=zn2;};
    ...
    begin
      ...
      else if ali <> nil then
      begin
        ali.FSel := false;
        ali.UpdateGraph;
      end;

      if (apo <> nil) or (ali <> nil) or (ssCtrl in Shift) then
      begin
        if (apo <> nil) and (atr <> nil) and (atr.FSelPoint >= 0) then
        begin
          atr.FSelPoint := -1;
          atr.UpdateGraph;
        end;
    ...
    if CurKey <> nil then
      CurKey.FCur := CurKey.NearestKey(Key_Time);
    ...
    else if (Triangle_Sel <> nil) and (Triangle_Sel.FSelPoint >= 0) then
    begin
      FMove := true;
      FMoveLP := Point(x, y);
...
begin
  if ((FMove) or (FRotateCamera)) and (Button = mbRight) then
  begin
    FMove := false;
    FRotateCamera := false;
...
begin
  ...
  begin
    ...
    begin
      while true do
      begin
        if not CalcWorld(FMoveLP,{0,}v1) then
          break;
        if not CalcWorld(Point(x, y),{0,}v2) then
          break;
    ...
    begin
      while true do
      begin
        if not CalcTexCoord(FMoveLP, tu1, tv1) then
          break;
        if not CalcTexCoord(Point(x, y), tu2, tv2) then
          break;
...
procedure TFormMain.EditPointPosKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
var
  apo: TPointAB;
begin
  ...
  begin
    ...
    if GetCountParEC(EditPointPos.Text, ',') >= 3 then
    begin
...
begin
  ...
  atr := Triangle_Sel;
  if GetCountParEC(EditTPTex.Text, ',') >= 2 then
  begin
...
procedure TFormMain.EditTPTexKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
  if Key = VK_RETURN then
    EditTPTexSelect(nil);
end;

procedure TFormMain.EditTPColorKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
var
  atr: TTriangleAB;
begin
  if FProgUpdate then
  ...
  if Key = VK_RETURN then
  begin
    if GetCountParEC(EditTPColor.Text, ',') >= 4 then
    begin
      atr.FV[atr.FSelPoint].FColor.VDWORD :=
        D3DCOLOR_ARGB(StrToIntEC(GetStrParEC(EditTPColor.Text, 0, ',')),
        StrToIntEC(GetStrParEC(EditTPColor.Text, 1, ',')), StrToIntEC(
        GetStrParEC(EditTPColor.Text, 2, ',')), StrToIntEC(GetStrParEC(EditTPColor.Text, 3, ',')));
...
begin
  ...
  atr.FV[atr.FSelPoint].FColor.VDWORD := D3DCOLOR_ARGB(ScrollBarTPA.Position, ScrollBarTPR.Position,
    ScrollBarTPG.Position, ScrollBarTPB.Position);
  atr.FV[atr.FSelPoint].FColor.Interpolate;
  atr.UpdateGraph;
  UpdateInfo;
  SortTriangle;
...
var
  atr: TTriangleAB;
begin
  if not OpenDialogTexture.Execute then
    exit;
  atr := Triangle_Sel;
...
var
  bp: TBlockParEC;
begin
  if (Sender <> nil) or (GFileName = '') then
  begin
...
begin
  ...
  begin
    ...
    while t < 1 do
    begin
      pt.x := pc.x + (p1.x - pc.x) * t;
      pt.y := pc.y + (p1.y - pc.y) * t;
      pt.z := pc.z + (p1.z - pc.z) * t;
      lpick[i] := PickTriangle(Point(Round(pt.x + GSmeX), Round(pt.y + GSmeY)));
      Inc(i);

      pt.x := pc.x + (p2.x - pc.x) * t;
      pt.y := pc.y + (p2.y - pc.y) * t;
      pt.z := pc.z + (p2.z - pc.z) * t;
      lpick[i] := PickTriangle(Point(Round(pt.x + GSmeX), Round(pt.y + GSmeY)));
      Inc(i);

      pt.x := pc.x + (p3.x - pc.x) * t;
      pt.y := pc.y + (p3.y - pc.y) * t;
      pt.z := pc.z + (p3.z - pc.z) * t;
      lpick[i] := PickTriangle(Point(Round(pt.x + GSmeX), Round(pt.y + GSmeY)));
      Inc(i);
...
*)
  ...
  begin
    ...
    begin
      ...
      while u < cntpick do
      begin
        if (lpick[u] <> nil) and (lpick[u].IndexOf(atr) > 0) then
          break;
        Inc(u);
      end;
      if u >= cntpick then
...
begin
  ...
  while atr <> nil do
  begin
    if (atr.FV[0].FVer.FSel) and (atr.FV[1].FVer.FSel) and (atr.FV[2].FVer.FSel) then
      for i := 0 to High(atr.FV) do
      begin
...
procedure TFormMain.EditLPColorKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
var
  ali: TLineAB;
  dw: DWORD;
begin
  ...
  if Key = VK_RETURN then
  begin
    if GetCountParEC(EditLPColor.Text, ',') >= 4 then
    begin
      dw := D3DCOLOR_ARGB(StrToIntEC(GetStrParEC(EditLPColor.Text, 0, ',')),
        StrToIntEC(GetStrParEC(EditLPColor.Text, 1, ',')), StrToIntEC(
        GetStrParEC(EditLPColor.Text, 2, ',')), StrToIntEC(GetStrParEC(EditLPColor.Text, 3, ',')));
      if ali.FSelPoint = 0 then
      begin
...
begin
  ...
  if ali.FSelPoint = 0 then
    FormKey.FKey := ali.FColorStart
  else
    FormKey.FKey := ali.FColorEnd;
  FormKey.ShowModal;
...
begin
  if Key_Copy = Triangle_Sel.FV[Triangle_Sel.FSelPoint].FColor then
    exit;
