...
begin
  CalcPosShow;
  if FGraph <> nil then
    FGraph.SetPos(D3DV(FPosShow.x + GSmeX, FPosShow.y + GSmeY, FPos.z));
end;

procedure TPointAB.SetPosC(x, y, z: single);
begin
  if (FPos.x = x) and (FPos.y = y) and (FPos.z = z) then
    exit;
  FPos := D3DV(x, y, z);
  CalcPosShow;
  if FGraph <> nil then
...
begin
  ...
  if FPortId <> '' then
    bp.Par_Add('PortId', FPortId);
  if FPortType <> '' then
    bp.Par_Add('PortType', FPortType);
  if FPortLink <> '' then
    bp.Par_Add('PortLink', FPortLink);
end;
