...
var
  ...
  r1, r2: single;
begin
  if (FVerStart = nil) or (FVerEnd = nil) then
    exit;
  ...
  FGraph.VerOpen;
  FGraph.Ver(0)^ := abVer3D(FVerStart.FPosShow.x + GSmeX, FVerStart.FPosShow.y + GSmeY,
    FVerStart.FPosShow.z, 0, 0, FColorStart.IpDWORD);
  FGraph.Ver(1)^ := abVer3D(FVerEnd.FPosShow.x + GSmeX, FVerEnd.FPosShow.y + GSmeY, FVerEnd.FPosShow.z,
    1, 1, FColorEnd.IpDWORD);
  FGraph.VerClose;
...
procedure Line_Clear;
begin
  while Line_First <> nil do
    Line_Delete(Line_Last);
end;
...
begin
  ...
  if Line_Last = el then
    Line_Last := el.FPrev;
  if Line_First = el then
    Line_First := el.FNext;
...
begin
  ...
  while el <> nil do
  begin
    if ((el.FVerStart = p1) and (el.FVerEnd = p2)) or ((el.FVerStart = p2) and (el.FVerEnd = p1)) then
    begin
      result := el;
      exit;
    end;
...
begin
  ...
  while el <> nil do
  begin
    if (el.FVerStart = p1) or (el.FVerEnd = p1) then
      li.Add(el);
