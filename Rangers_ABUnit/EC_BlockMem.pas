...
begin
	while FFirst<>nil do BlockDel(FLast);
end;
...
begin
	if (FLast=nil) or (size>(FLast.FMaxSize-FLast.FSize)) then begin
		BlockAdd();
        if size<=(FBlockSizeDefault shr 1) then begin
			FLast.FMaxSize:=FBlockSizeDefault;
        end else begin
			FLast.FMaxSize:=size;
        end;
		if FAllocClear then FLast.FBuf:=AllocClearEC(FLast.FMaxSize)
		else FLast.FBuf:=AllocEC(FLast.FMaxSize);
    end;
...
begin
    ...
    if el.FSize<=0 then BlockDel(el);
end;
